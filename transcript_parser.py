...
def _suggest_resolutions(data):
    ...
    options.append({
        ...
        "desc": "Structured monthly payments over up to 72 months. Use the IRS Pilot Navigator \u2014 \u201cInstallment Agreements\u201d \u2014 to see payment options and calculate your monthly amount.",
        "tag": "likely",
    })

    options.append({
        "name": "Offer in Compromise",
        "desc": "Settle for less than the full balance based on your ability to pay. Use the IRS Pilot Financial Intake Wizard to calculate your Reasonable Collection Potential and prepare Form 433-A OIC.",
        "tag": "analysis",
    })

    options.append({
        "name": "Currently Not Collectible (CNC)",
        "desc": "If monthly expenses exceed income, the IRS may defer collection. Use the IRS Pilot Navigator \u2014 \u201cCurrently Not Collectible\u201d \u2014 to evaluate whether you qualify.",
        "tag": "possible",
    })
    ...
    if penalties > 0:
        options.append({
            ...
            "desc": f"${penalties:,.2f} in penalties may be reducible. Use the IRS Pilot Letter Generator to draft your First-Time Abatement or Reasonable Cause request. Abating penalties also removes the interest charged on them.",
            "tag": "strong",
        })
    ...
    if data.get("has_cdp_hearing"):
        options.append({
            ...
            "desc": "The active CDP hearing is your formal opportunity to propose any resolution. Use the IRS Pilot Navigator \u2014 \u201cCollection Due Process\u201d \u2014 to prepare your proposal.",
            "tag": "active",
        })
...
def _build_faq(data):
    ...
    if balance > 0:
        faq.append({
            ...
            "a": (
                ...
                "The IRS Pilot Navigator explains each charge in detail under the \u201cPenalties & Interest\u201d section."
            )
        })
    ...
    if penalties > 0:
        faq.append({
            ...
            "a": (
                f"Possibly yes. The ${penalties:,.2f} in assessed penalties may be eligible for removal through "
                "a process called penalty abatement. The IRS offers First-Time Abatement for taxpayers with a "
                "clean compliance history, and Reasonable Cause abatement if there was a legitimate reason for "
                "the late filing or payment. If penalties are abated, the interest charged on those penalties "
                "is also removed. Use the IRS Pilot Letter Generator to draft your abatement request. "
                "If your situation is unusual or complex, a tax professional can help strengthen your case."
            )
        })
    ...
    if data.get("has_levy_notice") and not data.get("has_cdp_hearing") and not data.get("has_bankruptcy"):
        faq.append({
            ...
            "a": (
                "The IRS has issued a levy notice and may be able to seize wages or bank accounts. "
                "Open the IRS Pilot Navigator and go to the \u201cLevy & Seizure\u201d section immediately \u2014 "
                "it walks through your options step by step, including how to request a Collection Due Process "
                "hearing to stop the levy. Time is critical."
            )
        })
        ...
        faq.append({
            ...
            "a": (
                "No \u2014 not right now. The CDP hearing request and/or bankruptcy filing currently prevent "
                "the IRS from levying. However, this protection is temporary and interest continues to accrue "
                "daily. Use the IRS Pilot Navigator \u2014 go to \u201cCollection Due Process\u201d to understand "
                "your rights and prepare your resolution proposal before the hearing."
            )
        })
    ...
    if data.get("has_sfr"):
        faq.append({
            ...
            "a": (
                "Yes. Even though the IRS filed a Substitute for Return, you generally have the right to file "
                "your own original return, which may include deductions and credits the IRS did not include. "
                "This can significantly reduce the balance owed. Use the IRS Pilot Navigator \u2014 go to "
                "\u201cUnfiled Returns\u201d for guidance. If the year is complex or involves significant income, "
                "a tax professional can prepare the return for you."
            )
        })
    ...
    faq.append({
        ...
        "a": (
            "The IRS has a program called the Offer in Compromise that allows taxpayers to settle for less "
            "than the full amount owed, based on their ability to pay. Use the IRS Pilot Navigator to walk "
            "through the OIC eligibility requirements and use the Financial Intake Wizard to calculate your "
            "Reasonable Collection Potential \u2014 the key number the IRS uses to evaluate your offer. "
            "If you need help submitting the application, a tax professional can guide you through the process."
        )
    })
...
def _build_next_steps(data):
    """Build prioritized next steps directing users into IRS Pilot tools first."""
    steps = []
    balance = data.get("account_balance", 0)

    if data.get("has_levy_notice") and not data.get("has_cdp_hearing") and not data.get("has_bankruptcy"):
        steps.append(
            "Act immediately \u2014 the IRS has issued a levy notice and collection action may be imminent. "
            "Open the IRS Pilot Navigator and go to the \u201cLevy & Seizure\u201d section for step-by-step guidance on your options right now."
        )

    if data.get("has_cdp_hearing"):
        steps.append(
            "Prepare a resolution proposal for your CDP hearing. Use the IRS Pilot Navigator \u2014 go to "
            "\u201cCollection Due Process\u201d for a full walkthrough of what to present at the hearing, including "
            "installment agreement and Offer in Compromise options. If your situation is complex, a tax professional can represent you at the hearing."
        )

    if data.get("total_penalties_assessed", 0) > 0:
        steps.append(
            f"Request penalty abatement for the ${data['total_penalties_assessed']:,.2f} in assessed penalties. "
            "Use the IRS Pilot Letter Generator to draft your penalty abatement request \u2014 it includes "
            "First-Time Abatement and Reasonable Cause templates. If penalties are abated, the interest "
            "charged on those penalties is also removed."
        )

    steps.append(
        "Prepare your financial information. The IRS requires a complete picture of your income, expenses, "
        "assets, and liabilities before considering any resolution. Use the IRS Pilot Financial Intake Wizard "
        "to organize everything the IRS will ask for \u2014 it automatically applies IRS National and Local Standards "
        "and prepares Forms 433-A and 433-F."
    )

    steps.append(
        "Confirm that all other tax years are filed and current. The IRS will not enter into any resolution "
        "agreement until all required returns have been filed. Use the IRS Pilot Navigator \u2014 go to "
        "\u201cUnfiled Returns\u201d for guidance on getting into compliance."
    )

    if balance > 0:
        steps.append(
            "Evaluate whether an Offer in Compromise or installment agreement is the best resolution path. "
            "The IRS Pilot Navigator walks through both options in detail, including eligibility requirements "
            "and how to calculate your Reasonable Collection Potential (RCP). If you need personalized guidance "
            "on which option fits your specific situation, contact a tax professional."
        )
