...
def create_foundation_member(user_id: int, partner_id: int,
    ...
    """Elevate a referral partner to Foundation Member status."""
    try:
        with get_db() as conn:
            existing = conn.execute(
...
def get_foundation_member_by_user(user_id: int):
    """Check if a user is a Foundation Member."""
    with get_db() as conn:
        return conn.execute(
...
def list_foundation_members():
    """List all foundation members with their stats."""
    with get_db() as conn:
        return conn.execute(
...
def get_foundation_sponsor(partner_id: int):
    """Get the foundation member who sponsors this partner (if any)."""
    with get_db() as conn:
        return conn.execute(
...
def record_foundation_override(foundation_member_id: int, sale_amount: int,
    ...
    """Record a foundation member override commission."""
...
def get_foundation_recruited_partners(foundation_member_id: int):
    """List partners recruited by a foundation member."""
    with get_db() as conn:
        return conn.execute(
            "SELECT rp.*, u.email, "
