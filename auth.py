...
def init_db():
    ...
    migrations = [
        ...
        "ALTER TABLE pro_subscribers ADD COLUMN subdomain TEXT DEFAULT NULL",
    ]
    with get_db() as conn:
...
# Reserved words that can never be claimed as a subdomain
RESERVED_SUBDOMAINS = {
    "www", "app", "api", "admin", "mail", "email", "ftp", "blog",
    "static", "cdn", "assets", "help", "support", "docs", "status",
    "irspilot", "pro", "referral", "account", "login", "signup",
    "checkout", "billing", "navigator", "wizard", "demo", "test",
}

def validate_subdomain(slug: str):
    """Validate a proposed subdomain slug. Returns (clean_slug, error)."""
    import re
    slug = (slug or "").strip().lower()
    if not slug:
        return None, "Subdomain cannot be empty."
    if len(slug) < 3:
        return None, "Subdomain must be at least 3 characters."
    if len(slug) > 40:
        return None, "Subdomain must be 40 characters or fewer."
    if not re.match(r'^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$', slug):
        return None, "Subdomain can only contain lowercase letters, numbers, and hyphens (not at the start or end)."
    if slug in RESERVED_SUBDOMAINS:
        return None, f'"{slug}" is a reserved name and cannot be used.'
    return slug, None

def is_subdomain_available(slug: str, exclude_subscriber_id: int = None):
    with get_db() as conn:
        row = conn.execute(
            "SELECT id FROM pro_subscribers WHERE subdomain = ?", (slug,)
        ).fetchone()
    if not row:
        return True
    if exclude_subscriber_id and row["id"] == exclude_subscriber_id:
        return True
    return False

def set_pro_subdomain(subscriber_id: int, slug: str):
    """Validate, check availability, and assign a subdomain to a pro subscriber."""
    clean, err = validate_subdomain(slug)
    if err:
        return False, err
    if not is_subdomain_available(clean, exclude_subscriber_id=subscriber_id):
        return False, f'"{clean}" is already taken. Please choose another.'
    with get_db() as conn:
        conn.execute(
            "UPDATE pro_subscribers SET subdomain = ? WHERE id = ?",
            (clean, subscriber_id)
        )
    return True, None

def get_pro_subscriber_by_subdomain(slug: str):
    with get_db() as conn:
        return conn.execute(
            "SELECT * FROM pro_subscribers WHERE subdomain = ? AND active = 1",
            ((slug or "").strip().lower(),)
        ).fetchone()
