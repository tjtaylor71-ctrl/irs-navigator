...
def init_db():
    ...
    with get_db() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS bookkeeping_sessions (
                id           INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            CREATE INDEX IF NOT EXISTS idx_bk_sessions_created
                ON bookkeeping_sessions(created_by, created_at);

            CREATE TABLE IF NOT EXISTS bookkeeping_jobs (
    ...
    migrations = [
        ...
        "CREATE TABLE IF NOT EXISTS bookkeeping_categories (id INTEGER PRIMARY KEY AUTOINCREMENT, created_by INTEGER REFERENCES users(id), name TEXT NOT NULL, qbo_account TEXT DEFAULT \'\', created_at TEXT DEFAULT (datetime(\'now\')))",
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_bk_cat_name ON bookkeeping_categories(created_by, name)",
        "CREATE TABLE IF NOT EXISTS bookkeeping_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, created_by INTEGER REFERENCES users(id), match_type TEXT NOT NULL DEFAULT \'exact\', keyword TEXT NOT NULL, category TEXT NOT NULL, created_at TEXT DEFAULT (datetime(\'now\')))",
        "CREATE INDEX IF NOT EXISTS idx_bk_rules_user ON bookkeeping_rules(created_by, match_type)",
    ]
    with get_db() as conn:
