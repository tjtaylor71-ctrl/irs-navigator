...
function FormLink({ href, label }) {
  return React.createElement("a", {
    href: href,
    target: "_blank",
    rel: "noopener noreferrer",
    style: { display: "inline-flex", alignItems: "center", gap: 4, marginTop: 6, padding: "4px 10px", background: "#1a2d5a", color: "#7ec11f", borderRadius: 5, fontSize: 11, fontWeight: "bold", textDecoration: "none", letterSpacing: 0.3 }
  }, "\uD83D\uDCC4 " + (label || "Open Form"));
}

function FormLinkRow({ links }) {
  return React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginTop: 6 } },
    links.map(function(l, i) { return React.createElement(FormLink, { key: i, href: l.href, label: l.label }); })
  );
}
...
function ResetBtn({ onReset }) {
  return React.createElement("button", {
    onClick: onReset,
    style: { padding: "6px 14px", background: "#fff", border: "1.5px solid #ddd", borderRadius: 6, fontSize: 12, color: "#666", cursor: "pointer", fontFamily: "Georgia, serif" }
  }, "\u21BA Reset");
}
...
function WOTCCredit() {
  ...
  return React.createElement(StrategyShell, {
    ...
    steps: [
      React.createElement("div", null, "Before or on the day of hire, have the new employee complete IRS Form 8850 (Pre-Screening Notice) and ETA Form 9061 (Individual Characteristics Form).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/wotc-eligibility-checklist", label: "WOTC Eligibility Checklist" }] })),
...
function InstallmentSales() {
  ...
  return React.createElement(StrategyShell, {
    ...
    steps: [
      React.createElement("div", null, "Negotiate seller financing: Instead of receiving the full purchase price at closing, structure the deal so you receive a down payment and annual installments over several years.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/installment-sale-terms-worksheet", label: "Installment Sale Terms Worksheet" }] })),
...
const W2_CATEGORIES = [
  { id: "pretax", label: "\uD83C\uDFDB Pre-Tax Benefits" },
  { id: "deductions", label: "\uD83D\uDCCB Itemized Deductions" },
  { id: "investments", label: "\uD83D\uDCB9 Investment Strategy" },
  { id: "retirement", label: "\uD83C\uDFE6 Advanced Retirement" },
  { id: "realestate", label: "\uD83C\uDFE0 Real Estate" },
  { id: "compensation", label: "\uD83D\uDCBC Equity & Comp" },
  { id: "advanced", label: "\u2B50 Advanced Moves" },
];

const W2_STRATEGIES = {
  pretax: [
    { id: "w2_retirement", label: "Maximize Retirement Contributions" },
    { id: "w2_hsa", label: "HSA Strategy" },
    { id: "w2_fsa", label: "FSA Planning" },
    { id: "w2_ira", label: "Traditional IRA Deductibility" },
  ],
  deductions: [
    { id: "w2_tip_income", label: "Unreimbursed Employee Expenses" },
    { id: "w2_overtime", label: "Work-From-Home Deduction State" },
    { id: "w2_vehicle", label: "Vehicle / Commute Interest" },
    { id: "w2_salt", label: "SALT Itemization Strategy" },
  ],
  investments: [
    { id: "w2_taxloss", label: "Tax-Loss Harvesting" },
    { id: "w2_capgains", label: "Capital Gains Timing" },
  ],
  retirement: [
    { id: "w2_backdoor_roth", label: "Backdoor Roth IRA" },
    { id: "w2_mega_backdoor", label: "Mega Backdoor Roth" },
    { id: "w2_asset_location", label: "Asset Location Strategy" },
  ],
  realestate: [
    { id: "w2_str_material", label: "Short-Term Rental (Material Participation)" },
    { id: "w2_rep_status", label: "Real Estate Professional Status" },
    { id: "w2_passive_rental", label: "Passive Rental Loss Planning" },
  ],
  compensation: [
    { id: "w2_nqdc", label: "Nonqualified Deferred Comp (NQDC)" },
    { id: "w2_equity_comp", label: "Equity Compensation (RSU / ISO / NQ)" },
  ],
  advanced: [
    { id: "w2_clat", label: "Charitable Lead Annuity Trust (CLAT)" },
    { id: "w2_oil_gas", label: "Oil & Gas Working Interests" },
    { id: "w2_qsbs", label: "QSBS Exclusion (IRC \u00a71202)" },
  ],
};

// ── W2 Strategy Components ────────────────────────────────────────────────────

function W2RetirementContrib() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Maximize Retirement Contributions", irc: "IRC \u00a7401(k) / \u00a7403(b) / \u00a7457", onReset: reset,
    who: "W-2 employees with access to a 401(k), 403(b), or 457 plan. Also applies to those who want to layer in an IRA contribution on top of employer plan contributions.",
    steps: [
      React.createElement("div", null, "Elect the maximum employee deferral into your 401(k) or 403(b): $23,500 for 2025 ($31,000 if age 50+, or $34,750 if age 60\u201363 under SECURE 2.0 super catch-up).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/retirement-contribution-worksheet", label: "Retirement Contribution Worksheet" }] })),
      "If your employer offers a Roth 401(k) option, evaluate whether pre-tax or Roth contributions are better based on your current vs. expected future tax bracket.",
      "Contribute to a Traditional IRA if deductible (income limits apply for those covered by a workplace plan). If not deductible, evaluate a backdoor Roth instead.",
      "If you have a 457(b) plan (government or certain non-profit employers), this is a separate $23,500 limit — you can max both a 403(b) and 457(b) simultaneously.",
      "Verify your employer match schedule and contribute at least enough to capture the full match — this is an immediate 50\u2013100% return.",
      "Revisit your deferral rate each open enrollment. Auto-escalation features can help if manual changes are easy to forget."
    ],
    mistakes: [
      "Not contributing enough to get the full employer match \u2014 this is leaving guaranteed compensation on the table.",
      "Ignoring catch-up contributions after age 50 (or 60\u201363 for the new super catch-up) \u2014 the higher limits significantly accelerate tax-deferred growth.",
      "Assuming a Roth 401(k) is always better \u2014 if you\u2019re in a high bracket now and expect lower income in retirement, pre-tax deferral may save more."
    ]
  });
}

function W2HSAStrategy() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "HSA Strategy (Triple Tax Advantage)", irc: "IRC \u00a7223", onReset: reset,
    who: "W-2 employees enrolled in a High-Deductible Health Plan (HDHP). The HSA is arguably the most tax-efficient account available \u2014 contributions are pre-tax, growth is tax-free, and withdrawals for qualified medical expenses are tax-free.",
    steps: [
      React.createElement("div", null, "Confirm your health plan qualifies as an HDHP (2025: deductible \u2265 $1,650 individual / $3,300 family; out-of-pocket max \u2264 $8,300 / $16,600).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/hsa-contribution-tracker", label: "HSA Contribution Tracker" }] })),
      "Contribute the annual maximum: $4,300 individual / $8,550 family (2025). Add $1,000 catch-up if age 55+.",
      "Invest your HSA balance \u2014 don\u2019t leave it as cash. Most custodians allow investing in mutual funds once balance exceeds $1,000\u2013$2,000.",
      "Pay current medical expenses out-of-pocket if possible and save receipts indefinitely. You can reimburse yourself years later \u2014 there is no deadline for reimbursement.",
      "After age 65, HSA funds can be withdrawn for any reason (taxed as ordinary income, like a traditional IRA) \u2014 making the HSA a bonus retirement account.",
      "Never use HSA funds for non-medical expenses before age 65 \u2014 the penalty is 20% plus ordinary income tax."
    ],
    mistakes: [
      "Spending HSA funds immediately instead of investing and letting them compound tax-free.",
      "Contributing to an HSA while enrolled in Medicare or a non-HDHP plan \u2014 this triggers a penalty.",
      "Not keeping medical receipts \u2014 you need documentation to support tax-free reimbursements."
    ]
  });
}

function W2FSAPlanning() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "FSA Planning (Healthcare & Dependent Care)", irc: "IRC \u00a7125 / \u00a7129", onReset: reset,
    who: "W-2 employees with access to an employer\u2019s Section 125 cafeteria plan. FSAs reduce taxable wages dollar-for-dollar but have use-it-or-lose-it rules that require planning.",
    steps: [
      React.createElement("div", null, "Estimate your anticipated qualified medical expenses for the year. Contribute up to $3,300 (2025 Healthcare FSA limit) \u2014 no more than you expect to spend.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/fsa-planning-worksheet", label: "FSA Planning Worksheet" }] })),
      "For Dependent Care FSA: contribute up to $5,000 per household ($2,500 if married filing separately) for qualifying child or dependent care costs.",
      "Understand your employer\u2019s rollover or grace period rule: some plans allow a $660 rollover to the next year; others offer a 2.5-month grace period. There is no universal standard.",
      "FSA funds are available in full on day one of the plan year \u2014 you can incur the full $3,300 in January and the remaining payroll deductions fund it over the year.",
      "Use FSA funds for eligible expenses before the deadline: medical copays, prescriptions, dental, vision, and (for DCFSA) daycare, after-school programs, and summer day camps.",
      "If you\u2019re eligible for an HSA, do NOT enroll in a general-purpose Healthcare FSA \u2014 it disqualifies HSA eligibility. A limited-purpose FSA (dental/vision only) is allowed alongside an HSA."
    ],
    mistakes: [
      "Over-contributing and forfeiting unused funds at year-end \u2014 FSAs are not savings accounts.",
      "Enrolling in both a Healthcare FSA and an HSA \u2014 this disqualifies your HSA contributions.",
      "Missing the FSA spending deadline or claims submission deadline \u2014 unspent funds are forfeited to the employer."
    ]
  });
}

function W2IRADeductibility() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Traditional IRA Deductibility", irc: "IRC \u00a7219", onReset: reset,
    who: "W-2 employees who want to contribute to a Traditional IRA and deduct the contribution. Deductibility phases out for those covered by a workplace retirement plan at certain income levels.",
    steps: [
      React.createElement("div", null, "Determine if you (or your spouse) are covered by a workplace retirement plan \u2014 this triggers the income phase-out for IRA deductibility.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/ira-deductibility-worksheet", label: "IRA Deductibility Worksheet" }] })),
      "2025 phase-out ranges: Single covered by workplace plan: $79,000\u2013$89,000 MAGI. MFJ (covered spouse): $126,000\u2013$146,000. MFJ (non-covered spouse): $236,000\u2013$246,000.",
      "If your income is below the phase-out, contribute up to $7,000 ($8,000 if age 50+) and deduct the full amount.",
      "If your income is in the phase-out range, calculate the partial deduction. The remainder can be contributed as non-deductible \u2014 track it on Form 8606 to avoid double taxation later.",
      "If you\u2019re fully phased out, consider a backdoor Roth IRA instead (non-deductible Traditional IRA contribution immediately converted to Roth).",
      "Contribute by April 15 of the following year to count for the prior tax year."
    ],
    mistakes: [
      "Forgetting to file Form 8606 for non-deductible IRA contributions \u2014 without it, you\u2019ll pay tax again on withdrawals.",
      "Assuming IRA contributions are always deductible \u2014 the workplace plan coverage rule catches many high-income W-2 earners.",
      "Missing the April 15 contribution deadline for the prior year."
    ]
  });
}

function W2TipIncomeDeduction() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Unreimbursed Employee Expenses (State)", irc: "IRC \u00a7162 / State Law", onReset: reset,
    who: "W-2 employees in states that still allow a deduction for unreimbursed employee business expenses. The federal deduction was suspended through 2025 by the TCJA, but several states (CA, NY, PA, others) still allow it on the state return.",
    steps: [
      React.createElement("div", null, "Identify your state\u2019s treatment of unreimbursed employee expenses \u2014 check whether your state conforms to the post-TCJA federal suspension or still allows the deduction.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/tip-income-deduction-worksheet", label: "Employee Expense Worksheet" }] })),
      "Track all unreimbursed business expenses: home office (if required by employer), tools and equipment, professional licenses, uniforms, business travel not reimbursed, and professional education.",
      "For states allowing the deduction, expenses exceeding 2% of AGI are deductible (same threshold as the old federal rule). Document everything.",
      "Request reimbursement from your employer under an accountable plan \u2014 reimbursed amounts are excluded from income entirely, which is better than a deduction.",
      "If your employer won\u2019t reimburse, negotiate it into your compensation package. Employers can deduct the reimbursement as a business expense.",
      "Armed forces reservists, performing artists, and fee-basis government officials may still claim certain employee expenses on the federal return (Form 2106 above-the-line)."
    ],
    mistakes: [
      "Assuming the federal deduction still exists for ordinary W-2 employees \u2014 it was suspended through 2025 under TCJA.",
      "Skipping the state return claim for states that still allow it \u2014 this can mean hundreds or thousands in missed state deductions.",
      "Not requesting reimbursement from the employer first \u2014 reimbursement is always better tax treatment than a deduction."
    ]
  });
}

function W2OvertimeDeduction() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Work-From-Home Deduction (State)", irc: "State Law", onReset: reset,
    who: "W-2 remote workers in states that allow a home office deduction or that have sourcing rules favorable to remote workers. Also relevant for employees who work across state lines.",
    steps: [
      React.createElement("div", null, "Determine whether your state allows W-2 employees to deduct home office expenses. California, New York, and several others have their own rules separate from the federal suspension.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/overtime-deduction-worksheet", label: "Remote Work Deduction Worksheet" }] })),
      "If you work remotely for an out-of-state employer, understand the 'convenience of the employer' doctrine \u2014 some states (NY, CT, NE, PA) tax your remote wages if your employer is based there.",
      "Track days worked in each state throughout the year. Multi-state sourcing can affect both your liability and potential deductions.",
      "For state returns allowing home office: calculate the square footage ratio (office sq ft \u00f7 total home sq ft) and apply it to rent/mortgage interest, utilities, and insurance.",
      "Keep a contemporaneous log of remote workdays and the reason for working remotely (employer requirement vs. personal preference matters in some states).",
      "Consult your employer\u2019s HR or payroll department \u2014 multi-state employees often have withholding errors that create unexpected tax bills."
    ],
    mistakes: [
      "Assuming your home state is the only state you owe taxes to \u2014 the convenience doctrine can create liability in your employer\u2019s state.",
      "Not tracking workdays by state \u2014 without documentation, you cannot support apportionment claims.",
      "Claiming home office on the federal return as a W-2 employee \u2014 the federal deduction is suspended through 2025."
    ]
  });
}

function W2VehicleLoan() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Vehicle & Loan Interest Strategy", irc: "IRC \u00a7163", onReset: reset,
    who: "W-2 employees who own investment assets and have personal or vehicle loans. Investment interest is still deductible at the federal level; strategic debt placement can maximize deductible interest.",
    steps: [
      React.createElement("div", null, "Track all investment interest expense (interest paid on margin loans or loans used to purchase taxable investments). This is deductible up to net investment income on Schedule A.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/vehicle-loan-interest-log", label: "Vehicle & Loan Interest Log" }] })),
      "Personal loan interest (auto loans, personal loans, credit cards) is NOT deductible federally. However, if you have both a personal loan and investment assets, consider whether restructuring makes sense.",
      "Home equity loan interest is deductible only if the loan is used to buy, build, or substantially improve the home. Keep documentation of how proceeds were used.",
      "If you use a vehicle for investment purposes (traveling to meet with your investment advisor, visiting rental properties), track those miles separately \u2014 they may be deductible.",
      "Student loan interest remains deductible (up to $2,500) for those under the income phase-out threshold, even for W-2 employees who don\u2019t itemize.",
      "Consider paying down non-deductible consumer debt before investing in taxable accounts \u2014 the after-tax return must exceed the loan interest rate to justify the investment."
    ],
    mistakes: [
      "Deducting auto loan interest on the federal return \u2014 personal vehicle loan interest is not deductible.",
      "Missing the student loan interest deduction because you don\u2019t itemize \u2014 it\u2019s an above-the-line deduction available to everyone.",
      "Using home equity proceeds for non-home purposes and still claiming the interest deduction \u2014 the IRS requires tracing of funds."
    ]
  });
}

function W2SALTItemization() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "SALT Itemization Strategy", irc: "IRC \u00a7164", onReset: reset,
    who: "W-2 employees in high-tax states (CA, NY, NJ, IL, etc.) who pay significant state income tax and property taxes. The $10,000 SALT cap limits the federal deduction, but strategies exist to maximize the benefit.",
    steps: [
      React.createElement("div", null, "Calculate whether itemizing beats the standard deduction ($15,000 single / $30,000 MFJ for 2025). Add SALT (capped at $10,000), mortgage interest, charitable contributions, and other deductions.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/salt-itemization-worksheet", label: "SALT Itemization Worksheet" }] })),
      "The SALT cap applies to the combined total of state/local income taxes AND property taxes. If both together exceed $10,000, you\u2019re leaving deductions on the table at the federal level.",
      "Check if your state has a Pass-Through Entity (PTE) tax election \u2014 for business owners, this routes state tax through the entity and avoids the SALT cap entirely (SALT cap applies to individuals, not businesses).",
      "Bunch deductible expenses into high-income years: prepay property taxes by December 31, accelerate charitable contributions, and time large medical expenses to clear the 7.5% AGI floor.",
      "Consider a Donor-Advised Fund for bunching charitable contributions \u2014 fund the DAF in a high-deduction year, then distribute to charities over multiple years.",
      "If you\u2019re close to the itemizing threshold, coordinate with your spouse (if MFJ) on timing of deductible expenses and make sure estimated state tax payments are not overpaid."
    ],
    mistakes: [
      "Prepaying state income taxes into the following year hoping to deduct them early \u2014 you can only deduct state taxes when they are due and paid, not prepaid future-year taxes.",
      "Ignoring the PTE tax election if you own a pass-through business \u2014 this is a significant planning opportunity missed by many.",
      "Not bunching deductions \u2014 alternating between standard and itemized deductions across years can maximize total deductions over time."
    ]
  });
}

function W2TaxLossHarvesting() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Tax-Loss Harvesting", irc: "IRC \u00a71211 / \u00a71212", onReset: reset,
    who: "W-2 employees with taxable investment accounts (brokerage accounts outside of IRAs and 401(k)s). Tax-loss harvesting converts paper losses into real tax savings without requiring you to permanently exit your investment position.",
    steps: [
      React.createElement("div", null, "Review your taxable brokerage account for positions with unrealized losses, especially in Q4 or after market downturns.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/tax-loss-harvesting-log", label: "Tax-Loss Harvesting Log" }] })),
      "Sell the losing position to realize the loss. The loss first offsets any capital gains realized during the year. Up to $3,000 of excess losses can offset ordinary income annually.",
      "Immediately reinvest proceeds in a similar (but not substantially identical) security to maintain market exposure. This preserves your investment thesis while locking in the tax loss.",
      "Observe the wash-sale rule: do NOT buy the same or substantially identical security within 30 days before or after the sale. Violating this rule disallows the loss.",
      "Carry forward unused losses indefinitely. If you harvest $20,000 in losses but only have $5,000 in gains, the remaining $15,000 carries forward to future years.",
      "High-income earners also benefit from harvesting losses against the 3.8% Net Investment Income Tax (NIIT) \u2014 capital gains are subject to NIIT above $200,000 (single) / $250,000 (MFJ)."
    ],
    mistakes: [
      "Triggering the wash-sale rule by buying back the same ETF or fund within 30 days \u2014 this disallows the loss.",
      "Harvesting losses in a tax-deferred account (IRA, 401(k)) \u2014 losses inside retirement accounts have no tax benefit.",
      "Forgetting to track the adjusted cost basis of the replacement security \u2014 the disallowed wash-sale loss increases the basis of the replacement shares."
    ]
  });
}

function W2CapGainsTiming() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Capital Gains Timing", irc: "IRC \u00a71(h) / \u00a71222", onReset: reset,
    who: "W-2 employees with taxable investment accounts who have unrealized gains. Strategic timing of when you realize gains can reduce the tax rate from ordinary income rates (up to 37%) to preferential long-term capital gains rates (0%, 15%, or 20%).",
    steps: [
      React.createElement("div", null, "Identify positions with unrealized gains. Determine whether the holding period is short-term (under 1 year, taxed as ordinary income) or long-term (1+ year, preferential rates).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/capital-gains-timing-worksheet", label: "Capital Gains Timing Worksheet" }] })),
      "Wait for the 1-year holding period before selling appreciated positions. Converting a short-term gain to a long-term gain can cut the tax rate by more than half for high earners.",
      "2025 long-term capital gains rates: 0% (taxable income up to $48,350 single / $96,700 MFJ), 15% (up to $533,400 single / $600,050 MFJ), 20% above those thresholds.",
      "In low-income years (sabbatical, career transition, early retirement), harvest gains at 0% by recognizing capital gains up to the top of the 0% bracket.",
      "Consider installment sales for large asset sales \u2014 spreading gain recognition over multiple years can keep each year below the 20% threshold or avoid the NIIT.",
      "Donate highly appreciated securities to charity or a DAF instead of selling \u2014 you avoid the capital gains entirely and receive a fair market value charitable deduction."
    ],
    mistakes: [
      "Selling appreciated positions held just under 1 year \u2014 a few extra weeks can dramatically reduce the tax rate.",
      "Ignoring the 0% capital gains rate in low-income years \u2014 this is a valuable opportunity to step up basis at no tax cost.",
      "Triggering the Net Investment Income Tax (3.8%) by not accounting for it in the effective rate calculation."
    ]
  });
}

function W2BackdoorRoth() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Backdoor Roth IRA", irc: "IRC \u00a7408A / Notice 2014-54", onReset: reset,
    who: "High-income W-2 earners who are phased out of direct Roth IRA contributions (2025: $150,000 single / $236,000 MFJ phase-out begins). The backdoor Roth is a two-step workaround that is fully legal and IRS-acknowledged.",
    steps: [
      React.createElement("div", null, "Contribute to a Traditional IRA \u2014 non-deductible, after-tax. Limit: $7,000 ($8,000 if age 50+). Do NOT invest the funds yet.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/backdoor-roth-worksheet", label: "Backdoor Roth Worksheet" }] })),
      "Immediately convert the Traditional IRA to a Roth IRA. Because the contribution was non-deductible (no deduction taken), there is little to no taxable income on conversion.",
      "File Form 8606 with your return. Part I reports the non-deductible contribution; Part II reports the conversion. This creates a paper trail showing the basis and prevents double taxation.",
      "Watch for the pro-rata rule: if you have other pre-tax IRA funds (rollover IRAs, SEP-IRAs, SIMPLE IRAs), the IRS blends all IRA money together when calculating the taxable portion of a conversion. Consider rolling pre-tax IRAs into your employer 401(k) to clear the pro-rata problem.",
      "Repeat annually \u2014 the backdoor Roth is not a one-time strategy. Each year, contribute and immediately convert.",
      "Married couples can each do a backdoor Roth \u2014 $14,000 per year ($16,000 if both over 50) goes into Roth permanently."
    ],
    mistakes: [
      "Triggering the pro-rata rule by forgetting about existing pre-tax IRA balances \u2014 this makes part of the conversion taxable.",
      "Not filing Form 8606 \u2014 without it, the IRS will tax the conversion as fully taxable income.",
      "Investing the IRA funds before converting \u2014 if the market moves, you may have a taxable gain or loss complicating the conversion."
    ]
  });
}

function W2MegaBackdoor() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Mega Backdoor Roth", irc: "IRC \u00a7402(g) / \u00a7415", onReset: reset,
    who: "High-income W-2 employees whose 401(k) plan allows after-tax contributions and in-service withdrawals or in-plan Roth conversions. This strategy can move up to $46,000+ per year into Roth on top of normal 401(k) limits.",
    steps: [
      React.createElement("div", null, "Confirm your 401(k) plan allows: (1) after-tax (non-Roth) contributions beyond the $23,500 employee deferral, and (2) either in-service withdrawals or in-plan Roth conversions. Many plans do not allow this \u2014 check your Summary Plan Description.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/mega-backdoor-roth-checklist", label: "Mega Backdoor Roth Checklist" }] })),
      "The total 415(c) limit for 2025 is $70,000 (employee + employer contributions + after-tax). Subtract your regular deferral ($23,500) and employer match to find how much after-tax room remains.",
      "Contribute after-tax dollars up to the 415(c) limit. These contributions have no tax deduction but grow tax-deferred.",
      "As soon as possible after contributing, convert the after-tax balance to Roth (either via in-plan conversion or by rolling out to a Roth IRA on in-service withdrawal). The faster you convert, the less pre-tax earnings accumulate that would be taxable on conversion.",
      "After leaving the employer, you can roll the Roth 401(k) balance to a Roth IRA, preserving all tax-free growth.",
      "Consult your plan administrator or benefits department to confirm plan rules before implementing \u2014 the mechanics vary by plan."
    ],
    mistakes: [
      "Assuming all 401(k) plans allow after-tax contributions \u2014 many do not. Confirm with your plan documents before proceeding.",
      "Letting after-tax contributions sit and earn pre-tax earnings before converting \u2014 those earnings are taxable on conversion.",
      "Confusing Roth 401(k) contributions with after-tax contributions \u2014 they are different plan features with different tax treatment."
    ]
  });
}

function W2AssetLocation() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Asset Location Strategy", irc: "IRC \u00a71(h) / \u00a7408", onReset: reset,
    who: "W-2 employees with both taxable brokerage accounts and tax-advantaged accounts (401(k), IRA, Roth IRA). Asset location is about placing each investment in the account type where it will be taxed most favorably.",
    steps: [
      React.createElement("div", null, "Map out your accounts: taxable brokerage, traditional 401(k)/IRA (pre-tax), and Roth IRA/401(k) (after-tax). Each has a different tax profile on growth and withdrawals.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/asset-location-worksheet", label: "Asset Location Worksheet" }] })),
      "Place tax-inefficient assets in tax-deferred accounts: bonds, REITs, high-dividend stocks, and actively managed funds generate ordinary income best shielded in a 401(k) or traditional IRA.",
      "Place tax-efficient assets in taxable accounts: broad index funds (low turnover), buy-and-hold equities, and tax-managed funds. Long-term capital gains rates apply, and you control when to realize gains.",
      "Place highest-growth potential assets in Roth accounts: aggressive growth stocks, small-cap equity, and alternative investments where you expect the largest long-term appreciation benefit most from tax-free Roth treatment.",
      "Rebalance across account types with care \u2014 selling in taxable accounts triggers gains. Consider rebalancing primarily within tax-advantaged accounts.",
      "Asset location compounds in value over time \u2014 even a 0.5% improvement in after-tax return can amount to tens of thousands of dollars over a 20\u201330 year horizon."
    ],
    mistakes: [
      "Holding bonds in a taxable account when a 401(k) has room \u2014 bond interest is taxed as ordinary income and is better sheltered.",
      "Putting index funds only in retirement accounts and leaving high-turnover active funds in taxable accounts \u2014 the reverse of optimal.",
      "Treating all accounts as one pool when rebalancing and ignoring the tax cost of selling appreciated assets in taxable accounts."
    ]
  });
}

function W2STRMaterialParticipation() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Short-Term Rental (Material Participation)", irc: "IRC \u00a7469(c)(7)", onReset: reset,
    who: "W-2 employees who own short-term rental properties (average guest stay of 7 days or less). STRs can be exempt from the passive activity rules if the owner materially participates, allowing losses to offset W-2 income directly.",
    steps: [
      React.createElement("div", null, "Confirm your property qualifies as a short-term rental: average rental period of 7 days or less, OR average rental period of 31 days or less with significant personal services provided to guests.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/str-material-participation-log", label: "STR Material Participation Log" }] })),
      "Meet one of the IRS material participation tests for the STR activity. The most common: spend more than 500 hours in the activity during the year, OR spend more hours in this activity than any other business or rental activity.",
      "Document your hours meticulously \u2014 a contemporaneous log of time spent on guest communication, maintenance, booking management, and property oversight. This documentation is essential if audited.",
      "If you materially participate, STR losses (after depreciation, expenses, and interest) are non-passive and can offset your W-2 income dollar-for-dollar with no income limitation.",
      "Consider a cost segregation study on the STR property to front-load depreciation deductions in the early years of ownership.",
      "If you do NOT materially participate, STR losses are passive and can only offset passive income (other rental or passive activity income)."
    ],
    mistakes: [
      "Claiming material participation without a contemporaneous time log \u2014 the IRS will disallow losses on audit without documentation.",
      "Confusing STR material participation with Real Estate Professional Status \u2014 they are separate tests with different requirements.",
      "Not analyzing average rental period \u2014 a property with an 8-day average stay is a long-term rental under \u00a7469 rules, not an STR."
    ]
  });
}

function W2REPStatus() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Real Estate Professional Status", irc: "IRC \u00a7469(c)(7)", onReset: reset,
    who: "W-2 employees (or spouses of W-2 earners) who spend significant time in real estate activities. REPS allows rental losses to be treated as non-passive, offsetting W-2 income. Difficult to qualify while holding a full-time W-2 job, but spouses may qualify independently.",
    steps: [
      React.createElement("div", null, "Qualify as a Real Estate Professional: (1) more than 750 hours in real property trades or businesses in which you materially participate, AND (2) more than 50% of all personal services during the year are in those real estate activities.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/reps-hours-log", label: "REPS Hours Log" }] })),
      "For married couples: each spouse\u2019s hours are evaluated independently. If one spouse works part-time or is not employed, they may be able to qualify while the other maintains W-2 income.",
      "Once you qualify as a REPS, you must also materially participate in each rental activity (or elect to aggregate all rental activities into one for the material participation test).",
      "Make the grouping election on your tax return to aggregate all rental properties into a single activity. This makes it easier to meet the material participation threshold across all properties.",
      "Document every hour spent in real estate activities: property management, tenant communication, maintenance oversight, bookkeeping, looking for new properties, and studying real estate markets.",
      "REPS status is examined closely on audit. The IRS frequently challenges it for high-income taxpayers with W-2 jobs \u2014 the 750-hour and 50% tests are strictly enforced."
    ],
    mistakes: [
      "Claiming REPS while working a full-time W-2 job without clear documentation showing real estate hours exceed all other work hours.",
      "Not making the grouping election \u2014 without it, you must meet material participation separately for every property.",
      "Using estimated or reconstructed hour logs instead of contemporaneous records."
    ]
  });
}

function W2PassiveRentalLoss() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Passive Rental Loss Planning", irc: "IRC \u00a7469", onReset: reset,
    who: "W-2 employees with rental properties who do not qualify as Real Estate Professionals. Passive rental losses are generally suspended, but a $25,000 special allowance exists for active participants, and suspended losses can be unlocked through property sales.",
    steps: [
      React.createElement("div", null, "Determine if you qualify for the $25,000 rental loss special allowance: you must actively participate (making management decisions, approving tenants, setting rents) and your MAGI must be below $100,000 (phases out completely at $150,000).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/passive-rental-loss-worksheet", label: "Passive Rental Loss Worksheet" }] })),
      "If your MAGI is below $100,000, up to $25,000 of rental losses can offset ordinary income, including W-2 wages. Track this allowance annually.",
      "Suspended passive losses accumulate on Form 8582 each year. These losses are \u2018banked\u2019 and will be released when you sell the property or when you have passive income.",
      "When you sell a rental property, all accumulated suspended passive losses from that property are released and can offset the gain on sale, other passive income, or ordinary income.",
      "Passive income from other sources (other rental properties, partnership K-1 income, S-corp passive activities) can absorb passive losses each year. Consider whether your portfolio generates any passive income to offset against.",
      "Consider grouping rental activities if you own multiple properties \u2014 some groupings can create more passive income to absorb losses, but consult a tax professional before electing."
    ],
    mistakes: [
      "Forgetting that suspended losses are not lost forever \u2014 they carry forward indefinitely and become valuable at disposition.",
      "Exceeding the $100,000 MAGI threshold and assuming the $25,000 allowance is still available \u2014 it phases out completely at $150,000.",
      "Not tracking suspended losses on Form 8582 \u2014 without this form, losses are not properly carried forward."
    ]
  });
}

function W2NQDCPlanning() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Nonqualified Deferred Compensation (NQDC)", irc: "IRC \u00a7409A", onReset: reset,
    who: "Highly compensated W-2 employees (executives, senior management) whose employers offer NQDC plans. These plans allow deferring large amounts of compensation to future years, potentially at lower tax rates.",
    steps: [
      React.createElement("div", null, "Make your deferral election before the end of the year preceding the year the compensation will be earned. \u00a7409A requires elections to be irrevocable and made before the compensation is earned.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/nqdc-deferral-worksheet", label: "NQDC Deferral Worksheet" }] })),
      "Determine how much to defer: consider your current vs. expected future tax bracket, the financial health of your employer (NQDC is an unsecured obligation \u2014 you are an unsecured creditor if the company fails), and your distribution timing elections.",
      "Elect a distribution schedule: lump sum at a specific future date, installments upon separation from service, or upon a specified triggering event (retirement, disability, change of control). This election is generally irrevocable.",
      "NQDC assets remain on the employer\u2019s balance sheet. They are NOT protected from the employer\u2019s creditors. Evaluate the financial stability of your employer before deferring large amounts.",
      "FICA taxes (Social Security and Medicare) are assessed when the compensation vests, not when distributed. Plan for FICA impact in years of large vesting.",
      "Review the plan document for \u00a7409A compliance. Any violation results in immediate income inclusion plus a 20% excise tax plus interest \u2014 the penalties are severe."
    ],
    mistakes: [
      "Missing the irrevocable deferral election deadline \u2014 once compensation is earned or vested, deferral is no longer possible.",
      "Concentrating too much deferred compensation with a financially unstable employer \u2014 NQDC is unsecured and can be lost in bankruptcy.",
      "Confusing NQDC with qualified plan (401(k)) protections \u2014 qualified plans are protected from employer creditors; NQDC is not."
    ]
  });
}

function W2EquityComp() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Equity Compensation (RSU / ISO / NQ)", irc: "IRC \u00a783 / \u00a7422 / \u00a71(h)", onReset: reset,
    who: "W-2 employees who receive equity compensation: Restricted Stock Units (RSUs), Incentive Stock Options (ISOs), or Nonqualified Stock Options (NQSOs/NQs). Each has unique tax treatment requiring active planning.",
    steps: [
      React.createElement("div", null, "Understand your equity type: RSUs are taxed as ordinary income at vesting (W-2). ISOs may trigger AMT but qualify for capital gains treatment if holding requirements are met. NQs are taxed as ordinary income at exercise.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/equity-comp-worksheet", label: "Equity Compensation Worksheet" }] })),
      "RSU strategy: model the tax impact of large vesting events. Consider selling enough shares at vesting to cover the tax withholding (employers often withhold at the 22% supplemental rate, which may be insufficient for high earners).",
      "ISO strategy: exercise ISOs in low-income years to minimize AMT exposure. The AMT preference item is the spread between exercise price and FMV at exercise. Consider ISO exercises in December so you can monitor the full year\u2019s AMT position.",
      "NQSO strategy: time exercises to control ordinary income recognition. Avoid exercising large amounts in a year with other high income events.",
      "Qualifying ISO dispositions require holding the shares for at least 2 years from grant date AND 1 year from exercise date. Meeting both converts the gain to long-term capital gains rates.",
      "Concentrated positions in employer stock carry significant risk. Consider a gradual diversification strategy to reduce single-stock exposure while managing capital gains recognition."
    ],
    mistakes: [
      "Relying solely on employer\u2019s 22% RSU withholding when in the 32\u201337% bracket \u2014 this creates a large balance due at filing.",
      "Triggering a disqualifying ISO disposition by selling too early \u2014 losing the capital gains treatment and paying ordinary income rates instead.",
      "Holding too much employer stock for emotional or loyalty reasons \u2014 concentration risk can be financially catastrophic if the employer\u2019s stock declines."
    ]
  });
}

function W2CLATPlanning() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Charitable Lead Annuity Trust (CLAT)", irc: "IRC \u00a7170 / \u00a72522", onReset: reset,
    who: "High-income W-2 earners in peak earning years who want to make a large charitable impact now while passing appreciated assets to heirs later. CLATs are most effective in low interest rate environments.",
    steps: [
      React.createElement("div", null, "Work with an estate planning attorney to establish a CLAT. The trust makes fixed annual payments to one or more charities for a term of years. At the end of the term, remaining assets pass to your heirs.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/clat-planning-checklist", label: "CLAT Planning Checklist" }] })),
      "Fund the CLAT with highly appreciated assets (stock, real estate). The trust sells the assets without triggering capital gains at the trust level (as a charitable trust).",
      "You receive an upfront charitable income tax deduction equal to the present value of the charity\u2019s annuity stream. This can be a large deduction in a high-income year.",
      "The \u00a77520 rate (published monthly by IRS) determines the present value calculation. A higher \u00a77520 rate makes CLATs more favorable for income tax deductions; a lower rate is better for wealth transfer.",
      "Any investment return above the \u00a77520 rate accrues to the remainder beneficiaries (your heirs) free of gift tax. In strong markets, this can transfer substantial wealth tax-efficiently.",
      "CLATs are irrevocable \u2014 once funded, assets cannot be returned to you. Work with legal and financial advisors to confirm this strategy aligns with your long-term financial plan."
    ],
    mistakes: [
      "Funding a CLAT with cash instead of appreciated assets \u2014 you lose the capital gains avoidance advantage.",
      "Using a CLAT without understanding the irrevocability \u2014 the assets are permanently transferred out of your estate.",
      "Ignoring the \u00a77520 rate environment \u2014 CLATs work best when investment returns are expected to exceed the hurdle rate."
    ]
  });
}
...
function W2OilGas() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "Oil & Gas Working Interests", irc: "IRC \u00a7469(c)(3) / \u00a7263(c)", onReset: reset,
    who: "High-income W-2 earners seeking non-passive losses to offset ordinary income. Working interests in oil and gas are a statutory exception to the passive activity rules \u2014 losses are non-passive regardless of participation level.",
    steps: [
      React.createElement("div", null, "Understand the investment structure: a working interest (as opposed to a royalty interest) gives you an ownership stake in the drilling operation with unlimited liability for costs. This unlimited liability is what triggers the non-passive exception.", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/oil-gas-working-interest-checklist", label: "Oil & Gas Working Interest Checklist" }] })),
      "Intangible Drilling Costs (IDCs) \u2014 labor, fuel, supplies, and other costs incidental to drilling \u2014 are generally 100% deductible in the year incurred. For a $500,000 investment in a well with 70\u201380% IDCs, this creates $350,000\u2013$400,000 in deductions in year one.",
      "Tangible drilling costs (equipment, casing) are depreciated over time using MACRS. IDCs are immediately expensed under \u00a7263(c).",
      "Depletion: you may deduct either cost depletion or percentage depletion (15% of gross income from the well, for independent operators). Percentage depletion can exceed your basis in the property over time.",
      "Evaluate the operator\u2019s track record, the geological survey, and the economics of the well independently from the tax benefits. The investment must have a reasonable economic purpose beyond tax deductions.",
      "Be aware of the AMT: IDCs are an AMT preference item for integrated oil companies, but independent producers and royalty owners can deduct IDCs without AMT adjustment."
    ],
    mistakes: [
      "Investing purely for the tax deduction without evaluating the underlying economics \u2014 a dry hole is a permanent loss of capital.",
      "Holding the interest through a limited partnership \u2014 LP interests do not qualify for the working interest non-passive exception. You must hold a working interest directly or through an entity that doesn\u2019t limit liability.",
      "Ignoring the IDC AMT preference for investors subject to AMT \u2014 the deduction may be partially recaptured under the alternative minimum tax."
    ]
  });
}

function W2QSBSEligibility() {
  const reset = () => {};
  return React.createElement(StrategyShell, {
    title: "QSBS Exclusion (IRC \u00a71202)", irc: "IRC \u00a71202", onReset: reset,
    who: "W-2 employees (especially at startups) who hold or are offered equity in a qualifying small business. The \u00a71202 exclusion can eliminate up to 100% of capital gains tax on up to $10 million (or 10x basis) in gain \u2014 potentially saving millions.",
    steps: [
      React.createElement("div", null, "Verify QSBS eligibility at the time of stock issuance: (1) C-corporation, (2) aggregate gross assets did not exceed $50 million at issuance, (3) corporation is in a qualifying trade or business (excludes professional services, finance, hospitality, etc.).", React.createElement(FormLinkRow, { links: [{ href: "/planning/forms/qsbs-eligibility-worksheet", label: "QSBS Eligibility Worksheet" }] })),
      "The stock must be originally issued to you (not purchased on secondary market) in exchange for money, property, or services. ISOs and NSOs that are exercised can qualify if the other requirements are met.",
      "Hold the stock for more than 5 years from issuance date. This is a hard requirement \u2014 selling before 5 years disqualifies the exclusion (though a rollover under \u00a71045 may defer gain if reinvested in other QSBS within 60 days).",
      "For stock acquired after September 27, 2010, the exclusion is 100% of gain (no AMT adjustment). For earlier acquisitions, the exclusion may be 50% or 75%.",
      "The exclusion is per-taxpayer per-company: each spouse can exclude up to $10 million from the same company separately. Consider gifting QSBS to trusts or family members to multiply the exclusion.",
      "Document everything: the original issuance, the company\u2019s gross assets at time of issuance (get a written representation from the company), the business description, and your holding period."
    ],
    mistakes: [
      "Assuming LLC or S-corp equity qualifies \u2014 \u00a71202 applies only to C-corporation stock.",
      "Not confirming the company\u2019s gross asset test at issuance \u2014 if assets exceeded $50 million at the time you received stock, it does not qualify.",
      "Selling before the 5-year mark without using the \u00a71045 rollover \u2014 all gain becomes taxable without the benefit of planning."
    ]
  });
}

// ── W2 Tab Shell ─────────────────────────────────────────────────────────────
function W2WageEarnerTab() {
  const [cat, setCat] = React.useState("pretax");
  const [strat, setStrat] = React.useState("w2_retirement");
  const W2_COMPONENTS = {
    w2_retirement: W2RetirementContrib,
    w2_hsa: W2HSAStrategy,
    w2_fsa: W2FSAPlanning,
    w2_ira: W2IRADeductibility,
    w2_tip_income: W2TipIncomeDeduction,
    w2_overtime: W2OvertimeDeduction,
    w2_vehicle: W2VehicleLoan,
    w2_salt: W2SALTItemization,
    w2_taxloss: W2TaxLossHarvesting,
    w2_capgains: W2CapGainsTiming,
    w2_backdoor_roth: W2BackdoorRoth,
    w2_mega_backdoor: W2MegaBackdoor,
    w2_asset_location: W2AssetLocation,
    w2_str_material: W2STRMaterialParticipation,
    w2_rep_status: W2REPStatus,
    w2_passive_rental: W2PassiveRentalLoss,
    w2_nqdc: W2NQDCPlanning,
    w2_equity_comp: W2EquityComp,
    w2_clat: W2CLATPlanning,
    w2_oil_gas: W2OilGas,
    w2_qsbs: W2QSBSEligibility,
  };
  const StratComponent = W2_COMPONENTS[strat] || (function() { return null; });
  const switchCat = function(newCat) {
    setCat(newCat);
    setStrat(W2_STRATEGIES[newCat][0].id);
  };
  return React.createElement("div", { style: { display: "grid", gridTemplateColumns: "220px 1fr", gap: 20, alignItems: "start" } },
    React.createElement("div", null,
      W2_CATEGORIES.map(function(c) { return React.createElement("div", { key: c.id },
        React.createElement("div", { onClick: function() { switchCat(c.id); }, style: { padding: "10px 14px", fontWeight: "bold", fontSize: 12, color: cat === c.id ? "#7ec11f" : "#888", background: cat === c.id ? "#1a2d5a" : "transparent", borderRadius: 8, cursor: "pointer", marginBottom: 4, letterSpacing: 0.3 } }, c.label),
        cat === c.id && W2_STRATEGIES[c.id].map(function(s) { return React.createElement("div", { key: s.id, onClick: function() { setStrat(s.id); }, style: { padding: "8px 14px 8px 22px", fontSize: 13, color: strat === s.id ? "#1a2d5a" : "#555", fontWeight: strat === s.id ? "bold" : "normal", background: strat === s.id ? "#e8f5cc" : "transparent", borderRadius: 6, cursor: "pointer", marginBottom: 2, borderLeft: strat === s.id ? "3px solid #7ec11f" : "3px solid transparent" } }, s.label); })
      ); })
    ),
    React.createElement("div", { style: cardStyle }, React.createElement(StratComponent, null))
  );
...
function PlanningDashboard() {
  ...
  return React.createElement("div", { style: { fontFamily: "Georgia, serif", minHeight: "100vh", background: "#f8f6f1" } },
    ...
    React.createElement("div", { style: { maxWidth: 1200, margin: "0 auto", padding: "24px 24px 40px" } },
      ...
      activeTab === "w2" && React.createElement(W2WageEarnerTab, null),
