...
function DashboardPage() {
  ...
  return React.createElement(
    "div",
    { style: { fontFamily: "'DM Sans',sans-serif", background: "#f8f6f1", minHeight: "100vh" } },
    // Header
    ...
    React.createElement(
      ...
      // Pro Subscriber card
      user.isProSubscriber && user.proSubscriber && React.createElement(
        "div",
        { style: { ...card, background: "#f0fdf4", border: "1px solid #7ec11f" } },
        React.createElement("div", { style: { fontWeight: "bold", fontSize: 15, color: "#1a2d5a", marginBottom: 6 } }, "\u{1F3E2} Pro Subscription \u2014 " + (user.proSubscriber.firmName || "Your Firm")),
        React.createElement(
          "div",
          { style: { fontSize: 14, color: "#555", marginBottom: 14, lineHeight: 1.6 } },
          "Share this link with your clients \u2014 they'll see your firm's branding, not IRS Pilot's."
        ),
        React.createElement(
          "div",
          { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 14 } },
          React.createElement("code", { style: { flex: 1, minWidth: 200, background: "#fff", border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#1a2d5a", wordBreak: "break-all" } }, user.proSubscriber.clientLink),
          React.createElement("button", {
            onClick: () => { navigator.clipboard.writeText(user.proSubscriber.clientLink); },
            style: { padding: "10px 16px", background: "#15803d", color: "#fff", border: "none", borderRadius: 8, fontWeight: "bold", fontSize: 13, cursor: "pointer" }
          }, "\uD83D\uDCCB Copy Link")
        ),
        user.proSubscriber.subdomainLink && React.createElement(
          "div",
          { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 14, paddingTop: 12, borderTop: "1px solid #bbf7d0" } },
          React.createElement("div", { style: { fontSize: 12, color: "#15803d", fontWeight: "bold", width: "100%" } }, "\uD83C\uDF10 Your Custom Domain"),
          React.createElement("code", { style: { flex: 1, minWidth: 200, background: "#fff", border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#1a2d5a", wordBreak: "break-all" } }, user.proSubscriber.subdomainLink),
          React.createElement("button", {
            onClick: () => { navigator.clipboard.writeText(user.proSubscriber.subdomainLink); },
            style: { padding: "10px 16px", background: "#15803d", color: "#fff", border: "none", borderRadius: 8, fontWeight: "bold", fontSize: 13, cursor: "pointer" }
          }, "\uD83D\uDCCB Copy Link")
        ),
        React.createElement(
          ...
          React.createElement("strong", { style: { color: "#1a2d5a" } }, (user.proSubscriber.sessionsUsed || 0) + " / " + (user.proSubscriber.sessionsLimit || 0))
        )
      ),
      // Referral card
      React.createElement(
