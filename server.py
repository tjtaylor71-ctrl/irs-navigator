...
<body>
  ...
  <script>
    try {
      ...
      document.body.innerHTML = "<div style=\"padding:40px;font-family:sans-serif;color:#dc2626\"><h2>Error</h2><pre>" + e.stack + "</pre></div>";
    }
  </script>
</body>
</html>"""
...
def serve_js(filename):
    ...
    # Strip any query string parameters (e.g. ?v=timestamp for cache busting)
    clean_filename = filename.split('?')[0]
    js_path = BASE_DIR / clean_filename
    if not js_path.exists() or not clean_filename.endswith('.js'):
