...
def api_pro_signup_complete():
    ...
    # Auto-create Stripe subscription
    payment_url = None
    try:
        stripe_sub_id, payment_url, customer_id, stripe_err =             stripe_checkout.create_pro_subscription_direct(user_id, email, sub_id, firm_name)
        if not stripe_err and customer_id:
            with auth.get_db() as conn:
                conn.execute(
                    "UPDATE pro_subscribers SET stripe_sub_id = ?, stripe_customer_id = ? WHERE id = ?",
                    (stripe_sub_id, customer_id, sub_id)
    ...
    # Send welcome email
    try:
        email_service.send_pro_approval_letter(contact_name, email, firm_name)
    except Exception:
        pass
