...
def login_page():
    ...
    html = html.replace(
        "<script>",
        f"<script>window.__LOGIN_PRODUCT__ = {repr(product)}; window.__LOGIN_MODE__ = {repr(mode)};</script>\n  <script>",
        1
    )
...
def api_register():
    ...
    resp  = make_response(jsonify({"redirect": f"/checkout?product={product}" if product else "/"}))
...
def checkout():
    ...
    # Stripe integration coming in Session 3 — for now show a placeholder
    product = request.args.get("product", "bundle")
    prices  = {"navigator": "$59", "wizard": "$99", "bundle": "$129"}
    price   = prices.get(product, "$59")
    html = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Checkout</title>
<style>body{{font-family:Georgia,serif;background:#f8f6f1;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}}
.box{{background:#fff;border-radius:16px;border:1px solid #e8e4dc;padding:40px;max-width:480px;text-align:center}}
h2{{color:#1a1a2e;margin-bottom:8px}}p{{color:#666;margin-bottom:24px}}
.btn{{display:inline-block;padding:14px 28px;background:#1a1a2e;color:#c8a96e;border:2px solid #c8a96e;border-radius:8px;font-family:Georgia,serif;font-weight:bold;font-size:15px;text-decoration:none}}</style>
</head><body><div class="box">
<div style="font-size:48px;margin-bottom:16px">💳</div>
<h2>Complete Your Purchase</h2>
<p>You're one step away from accessing your IRS tools.<br><strong>{product.title()} — {price} / 7 days</strong></p>
<p style="font-size:13px;color:#aaa">Stripe payment integration coming very soon. In the meantime, <a href="https://www.calendly.com/taylor-tax-financial/tax-help" style="color:#1a1a2e">schedule a consultation</a> to get access immediately.</p>
