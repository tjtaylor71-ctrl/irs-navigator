...
def api_admin_send_approval_letter():
    """Send approval/welcome letter to a pro interest submission."""
    data  = request.get_json(force=True, silent=True) or {}
    name  = data.get("name", "")
    email = data.get("email", "")
    firm  = data.get("firm", "")
    try:
        email_service.send_pro_approval_letter(name, email, firm)
        with auth.get_db() as conn:
            conn.execute("UPDATE taxpro_interests SET status = 'approved' WHERE id = ?", (data.get("id"),))
        return jsonify({"sent": True})
    except Exception as e:
