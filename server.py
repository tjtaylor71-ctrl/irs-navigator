...
PAGE_NO_NAV = {"LoginPage", "ResetPage", "TOSPage", "PrivacyPage", "CovidPenaltyTool", "BookkeepingTool", "ProSignupPage"}
...
def api_bookkeeping_extract():
    ...
    try:
        return _api_bookkeeping_extract_inner()
    except Exception as _top_e:
        app.logger.error(f"Bookkeeping top-level crash: {traceback.format_exc()}")
        return jsonify({"error": f"Unhandled: {type(_top_e).__name__}: {str(_top_e)[:300]}", "trace": traceback.format_exc()[-800:]}), 500

def _api_bookkeeping_extract_inner():
    import urllib.request, urllib.error, json as json_lib, re, base64 as b64lib, io
...
def api_generate_letter():
    ...
    prompt = f"""Write a formal IRS correspondence letter. The letter is written in FIRST PERSON — the taxpayer is writing directly to the IRS themselves, not through a representative.
...
def seed_admin():
    ...
    try:
        ...
        with auth.get_db() as conn:
            ...
            # Also ensure any other admin emails get covid_penalty access
            for admin_email in ADMIN_EMAILS:
                admin_row = conn.execute("SELECT id FROM users WHERE email = ?", (admin_email,)).fetchone()
                if admin_row and admin_row["id"] != user_id:
                    ex = conn.execute(
                        "SELECT id FROM purchases WHERE user_id = ? AND product = 'covid_penalty' AND stripe_session_id = 'admin_permanent'",
                        (admin_row["id"],)
                    ).fetchone()
                    if ex:
                        conn.execute("UPDATE purchases SET expires_at = ? WHERE id = ?", (long_expiry, ex["id"]))
                    else:
                        conn.execute(
                            "INSERT INTO purchases (user_id, product, stripe_session_id, expires_at) VALUES (?, 'covid_penalty', 'admin_permanent', ?)",
                            (admin_row["id"], long_expiry)
                        )
