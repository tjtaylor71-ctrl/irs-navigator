...
- If you don't know something, direct them to info@taylortaxandfinancial.com"""
    ...
    try:
        ...
        reply = result.content[0].text if result.content else             "I had trouble processing that. Please email info@taylortaxandfinancial.com."
        return jsonify({"reply": reply})
    except Exception as e:
        app.logger.error(f"Chat API error: {e}")
        return jsonify({"reply": f"I'm having trouble right now. Please email info@taylortaxandfinancial.com for help. (Error: {str(e)[:100]})"})
