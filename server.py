  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 640px) {
      /* Prevent any element from overflowing the screen */
      body, #root, #root > div { overflow-x: hidden; max-width: 100vw; }

      /* Header nav — hide text labels, show icons only, wrap gracefully */
      ...
      /* Stack any side-by-side grids to single column */
      .mobile-stack {
        flex-direction: column !important;
        grid-template-columns: 1fr !important;
      }

      /* Force all auto-fit grids to single column on small phones */
      [style*="repeat(auto-fit"] {
        grid-template-columns: 1fr !important;
      }

      /* Action buttons full width on mobile */
      [style*="requireAccess"] { width: 100% !important; }
...
<body>
  <div id="root"><div style="padding:40px;font-family:monospace;font-size:14px;color:#555">Loading...</div></div>
  <script>
    window.onerror = function(msg, src, line, col, err) {
      document.body.style.background = '#fff';
      document.body.innerHTML = '<div style="padding:40px;font-family:monospace;color:#dc2626;background:#fff">'
        + '<h2 style="margin-bottom:16px">JavaScript Error</h2>'
        + '<b>Message:</b> ' + msg + '<br><br>'
        + '<b>Source:</b> ' + src + '<br>'
        + '<b>Line:</b> ' + line + ', Col: ' + col + '<br><br>'
        + (err ? '<b>Stack:</b><pre>' + err.stack + '</pre>' : '')
        + '</div>';
      return true;
    };
    window.addEventListener('unhandledrejection', function(e) {
      document.body.style.background = '#fff';
      document.body.innerHTML = '<div style="padding:40px;font-family:monospace;color:#dc2626;background:#fff">'
        + '<h2>Unhandled Promise Rejection</h2><pre>' + (e.reason?.stack || e.reason) + '</pre></div>';
    });
  </script>
  <script src="/js/COMPILED_FILE"></script>
  <script>
    try {
      if (!window.COMPONENT_NAME) throw new Error('Component window.COMPONENT_NAME not found in COMPILED_FILE. The file may have a syntax error or failed to load.');
      const domContainer = document.getElementById('root');
      const root = ReactDOM.createRoot(domContainer);
      root.render(React.createElement(window.COMPONENT_NAME));
    } catch(e) {
      document.body.style.background = '#fff';
      document.body.innerHTML = '<div style="padding:40px;font-family:monospace;color:#dc2626;background:#fff"><h2>App Error</h2><pre>' + e.stack + '</pre></div>';
    }
  </script>
</body>
</html>"""
...
def build_page(compiled_file, component_name, title):
    ...
    import time
    compiled_path2 = BASE_DIR / compiled_file
    version = int(compiled_path2.stat().st_mtime) if compiled_path2.exists() else int(time.time())
    page = page.replace(f'src="/js/{compiled_file}"', f'src="/js/{compiled_file}?v={version}"')
