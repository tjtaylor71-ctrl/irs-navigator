...
def api_me():
    ...
    return jsonify({"loggedIn": True, "email": user["email"], "access": access, "wizardLocked": wizard_locked})
...
- Keep answers conversational and easy to read - write in plain sentences, not markdown
- Never use markdown formatting - no headers (##), no bold (**), no bullet dashes, no horizontal rules (---)
- Break your response into short paragraphs of 2-3 sentences each - never write one long block of text
- Each paragraph should cover one idea - pricing in one paragraph, features in another, how to get started in another
- For lists of items, put each item on its own line with no dash or bullet symbol
- Keep the total response concise - 3 short paragraphs maximum for most questions
- Be warm and professional, like a helpful staff member
...
# ── Seed admin account ─────────────────────────────────────────────────────────
def seed_admin():
    """Create admin account if it doesn't exist. Safe to run on every startup."""
    ...
    try:
        ...
        if not exists:
            ...
            app.logger.info("Admin account already exists — skipping seed.")
    except Exception as e:
