...
def api_bookkeeping_extract():
    ...
    payload = json_lib.dumps({
        ...
        "system": (
            ...
            f"category (one of: {', '.join(CATS)}), "
            "flag (\"ok\" if clearly identifiable, \"unclear\" if ambiguous), "
            "note (string: empty or brief reason if unclear)."
        ),
        "messages": [{
            "role": "user",
            "content": f"Extract all transactions from this bank statement as a JSON array only.\n\nFilename: {filename}\n\n{text}"
        }]
    }).encode("utf-8")
...
@require_admin
def api_admin_data():
    PRODUCT_PRICES = {"navigator": 59, "wizard": 99, "bundle": 129, "planning": 99}
...
def api_admin_grant():
    ...
    email   = data.get("email", "").strip().lower()
    product = data.get("product", "")
    if not email or product not in ("navigator", "wizard", "bundle", "planning"):
...
def api_validate_discount():
    ...
    PRODUCT_AMOUNTS = {"navigator": 5900, "wizard": 9900, "bundle": 12900, "planning": 9900}
...
BASE_PRICES = {"navigator": 5900, "wizard": 9900, "bundle": 12900, "planning": 9900}
