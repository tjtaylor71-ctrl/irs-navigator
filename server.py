...
# ── HTML Shell ─────────────────────────────────────────────────────────────────
# A minimal HTML page that loads React + Babel from CDN and renders the wizard.
HTML_SHELL = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>IRS Navigator — Taxpayer Self-Help</title>
...
<body>
  <div id="loading">
    ...
    <div style="font-size:15px;color:#7f8c8d;">Loading IRS Navigator…</div>
  </div>
  <div id="root"></div>

  <script type="text/babel" src="/static/irs-intake-wizard.jsx" data-type="module"></script>
  <script type="text/babel">
    // Mount the wizard once the JSX is parsed
    window.addEventListener('load', () => {
      const root = ReactDOM.createRoot(document.getElementById('root'));
      document.getElementById('loading').style.display = 'none';
      root.render(React.createElement(IRSIntakeWizard));
    });
  </script>
</body>
</html>
...
@app.route("/")
def index():
    """Serve the wizard shell page."""
    return Response(HTML_SHELL, mimetype="text/html")
