              ...
              /* @__PURE__ */ React.createElement("tr", { key: p.id, style: { borderTop: "1px solid #f0ede8" } },
                ...
                /* @__PURE__ */ React.createElement("td", { style: { padding: "12px 16px" } },
                  /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 5, flexWrap: "wrap" } },
                    ...
                    p.active && /* @__PURE__ */ React.createElement("button", { onClick: async () => { if (!confirm("Resend referral link, Partner Guide + W-9 to " + p.email + "?")) return; const res = await fetch("/api/admin/referral/resend-documents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: p.id }) }); const d = await res.json(); alert(res.ok ? "\u2705 " + d.message : "\u274C " + (d.error || d.warning || "")); }, style: { padding: "4px 8px", background: "transparent", border: "1px solid #bfdbfe", color: "#1e40af", borderRadius: 5, fontSize: 11, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" } }, "\uD83D\uDCE7 Resend Docs"),
