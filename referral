...
function ReferralPage() {
  ...
  ].map((s) => /* @__PURE__ */ React.createElement("div", { key: s.label, style: { background: "#fff", borderRadius: 12, border: "1px solid #e8e4dc", padding: "20px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 26, fontWeight: "bold", color: "#1a2d5a" } }, s.value), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#888", marginTop: 4 } }, s.label)))), partner && React.createElement(ProfileEditor, { partner: partner, onSaved: function(){ fetch("/api/referral/profile", { credentials:"include" }).then(function(r){return r.json();}).then(function(d){if(d&&d.ok&&d.profile)setPartner(function(prev){ return Object.assign({},prev,d.profile); });}); } }),
...
function ProfileEditor(props) {
  ...
  var _p = partner || {};
  var nState = React.useState(_p.name || ""); var name = nState[0]; var setName = nState[1];
  var fState = React.useState(_p.firm || ""); var firm = fState[0]; var setFirm = fState[1];
  var pState = React.useState(_p.phone || ""); var phone = pState[0]; var setPhone = pState[1];
  var cState = React.useState(_p.credential || ""); var cred = cState[0]; var setCred = cState[1];
  var wState = React.useState(_p.website || ""); var website = wState[0]; var setWebsite = wState[1];
  var aState = React.useState(_p.address || ""); var address = aState[0]; var setAddress = aState[1];
  ...
  if (!open) return React.createElement("div", { style:{ background:"#fff", border:"1px solid #e8e4dc", borderRadius:12, padding:"16px 20px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center" } },
    React.createElement("div", null,
      React.createElement("div", { style:{ fontWeight:"bold", color:"#1a2d5a", fontSize:14 } }, _p.name || "Complete your profile"),
      React.createElement("div", { style:{ fontSize:12, color:"#888", marginTop:2 } }, (_p.firm||"") + (_p.credential ? " \u00b7 " + _p.credential : "") + (_p.phone ? " \u00b7 " + _p.phone : ""))
    ),
