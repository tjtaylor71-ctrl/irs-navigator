...
function ReferralPage() {
  ...
  if (loading) return React.createElement("div", { style: { fontFamily: "'DM Sans',sans-serif", padding: 40, color: "#888" } }, "Loading...");
  ...
  ].map((s) => /* @__PURE__ */ React.createElement("div", { key: s.label, style: { background: "#fff", borderRadius: 12, border: "1px solid #e8e4dc", padding: "20px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 26, fontWeight: "bold", color: "#1a2d5a" } }, s.value), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: "#888", marginTop: 4 } }, s.label)))), /* @__PURE__ */ React.createElement(ProfileEditor, { partner: partner, onSaved: function(){ fetch("/api/referral/partner", { credentials:"include" }).then(function(r){return r.json();}).then(function(d){if(d.partner)setPartner(d.partner);}); } }),
  React.createElement("div", { style: card }, /* @__PURE__ */ React.createElement("div", { style: { fontWeight: "bold", fontSize: 15, marginBottom: 12, color: "#1a2d5a" } }, "Your Referral Link"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, padding: "10px 14px", background: "#f8f6f1", borderRadius: 8, border: "1px solid #e8e4dc", fontSize: 14, color: "#555", wordBreak: "break-all" } }, BASE_URL, "/refer/", partner.code), /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: copyLink,
      style: { padding: "10px 18px", background: copied ? "#7ec11f" : "#1a2d5a", color: copied ? "#1a2d5a" : "#7ec11f", border: "2px solid #7ec11f", borderRadius: 8, fontFamily: "'DM Sans',sans-serif", fontWeight: "bold", fontSize: 14, cursor: "pointer", whiteSpace: "nowrap" }
    },
...
function ProfileEditor(props) {
  var partner = props.partner; var onSaved = props.onSaved;
  ...
  var inp = { width:"100%", border:"1px solid #e8e4dc", borderRadius:8, padding:"9px 12px", fontSize:14, fontFamily:"'DM Sans',sans-serif", boxSizing:"border-box", marginBottom:12 };
  var lbl = { display:"block", fontSize:11, fontWeight:"bold", color:"#1a2d5a", marginBottom:3, textTransform:"uppercase" };

  function save() {
    setSaving(true); setMsg("");
    fetch("/api/referral/profile", { method:"POST", credentials:"include",
      headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ name:name, firm:firm, phone:phone, credential:cred, website:website, address:address })
    }).then(function(r){ return r.json(); }).then(function(d){
      setSaving(false);
      if (d.ok) { setMsg("\u2705 Profile updated successfully"); if(onSaved) onSaved(); setTimeout(function(){ setOpen(false); setMsg(""); }, 1500); }
      else { setMsg("\u274C " + (d.error||"Error saving")); }
    });
  }

  if (!open) return React.createElement("div", { style:{ background:"#fff", border:"1px solid #e8e4dc", borderRadius:12, padding:"16px 20px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center" } },
    React.createElement("div", null,
      React.createElement("div", { style:{ fontWeight:"bold", color:"#1a2d5a", fontSize:14 } }, partner.name || "Complete your profile"),
      React.createElement("div", { style:{ fontSize:12, color:"#888", marginTop:2 } }, (partner.firm||"") + (partner.credential ? " \u00b7 " + partner.credential : "") + (partner.phone ? " \u00b7 " + partner.phone : ""))
    ),
    React.createElement("button", { onClick:function(){setOpen(true);}, style:{ background:"#f0f7ff", color:"#1a2d5a", border:"1px solid #1a2d5a", borderRadius:7, padding:"7px 14px", fontFamily:"'DM Sans',sans-serif", fontSize:13, cursor:"pointer", fontWeight:"bold" } }, "\u270F\uFE0F Edit Profile")
  );

  return React.createElement("div", { style:{ background:"#fff", border:"2px solid #1a2d5a", borderRadius:12, padding:"24px 24px", marginBottom:16 } },
    React.createElement("div", { style:{ fontWeight:"bold", fontSize:16, color:"#1a2d5a", marginBottom:16 } }, "Edit Your Profile"),
    React.createElement("div", { style:{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:12, marginBottom:4 } },
      React.createElement("div", null, React.createElement("label", { style:lbl }, "Full Name"), React.createElement("input", { style:inp, value:name, onChange:function(e){setName(e.target.value);} })),
      React.createElement("div", null, React.createElement("label", { style:lbl }, "Firm Name"), React.createElement("input", { style:inp, value:firm, onChange:function(e){setFirm(e.target.value);} })),
      React.createElement("div", null, React.createElement("label", { style:lbl }, "Phone"), React.createElement("input", { style:inp, value:phone, onChange:function(e){setPhone(e.target.value);} })),
      React.createElement("div", null, React.createElement("label", { style:lbl }, "Credential"), React.createElement("input", { style:inp, placeholder:"EA, CPA...", value:cred, onChange:function(e){setCred(e.target.value);} })),
      React.createElement("div", { style:{ gridColumn:"1/-1" } }, React.createElement("label", { style:lbl }, "Website"), React.createElement("input", { style:inp, placeholder:"https://yourfirm.com", value:website, onChange:function(e){setWebsite(e.target.value);} })),
      React.createElement("div", { style:{ gridColumn:"1/-1" } }, React.createElement("label", { style:lbl }, "Business Address"), React.createElement("input", { style:inp, placeholder:"123 Main St, City, State ZIP", value:address, onChange:function(e){setAddress(e.target.value);} }))
    ),
    msg && React.createElement("div", { style:{ padding:"10px 14px", borderRadius:7, fontSize:13, marginBottom:10, color:msg.startsWith("\u2705")?"#15803d":"#dc2626", background:msg.startsWith("\u2705")?"#f0fdf4":"#fef2f2", border:"1px solid "+(msg.startsWith("\u2705")?"#7ec11f":"#fca5a5") } }, msg),
    React.createElement("div", { style:{ display:"flex", gap:10 } },
      ...
      React.createElement("button", { onClick:function(){setOpen(false);setMsg("");}, style:{ background:"transparent", color:"#aaa", border:"1px solid #e8e4dc", borderRadius:8, padding:"11px 16px", fontFamily:"'DM Sans',sans-serif", fontSize:13, cursor:"pointer" } }, "Cancel")
    )
  );
}
