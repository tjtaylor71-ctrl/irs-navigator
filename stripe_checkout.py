...
FOUNDATION_PRICE   = os.environ.get("STRIPE_FOUNDATION_PRICE", "")  # $99/month Foundation
...
def create_foundation_checkout(fm_id: int, user_email: str,
                               success_url: str, cancel_url: str) -> tuple:
    """Create a Stripe checkout session for Foundation $99/month subscription."""
    import stripe as _stripe
    _stripe.api_key = STRIPE_SECRET
    if not FOUNDATION_PRICE:
        return None, "Foundation Stripe price not configured. Set STRIPE_FOUNDATION_PRICE in Railway."
    try:
        session = _stripe.checkout.Session.create(
            payment_method_types=["card"],
            mode="subscription",
            customer_email=user_email,
            line_items=[{"price": FOUNDATION_PRICE, "quantity": 1}],
            success_url=success_url,
            cancel_url=cancel_url,
            metadata={"type": "foundation", "fm_id": str(fm_id)},
        )
        return session.url, None
    except Exception as e:
        return None, str(e)
