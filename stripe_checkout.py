...
PRICE_TO_PRODUCT = {
    ...
    "price_1TWewaKCmkbEhj2EBxJajrQw": "covid_penalty",
    # Test mode
...
PRODUCT_TO_PRICE = {
    ...
    "covid_penalty": "price_1TWewaKCmkbEhj2EBxJajrQw",
}
...
PRODUCT_NAMES = {
    ...
    "covid_penalty": "COVID Penalty Relief Tool — 7-Day Access",
}
...
PRODUCT_REDIRECT = {
    ...
    "covid_penalty": "/covid-penalty-relief",
}
...
def create_pro_signup_checkout(user_id: int, user_email: str,
                                subscriber_id: int, firm_name: str,
                                signup_token: str):
    """
    Create a Stripe Checkout session for pro subscription signup.
    Subscriber pays immediately via hosted Checkout page.
    Uses subscription schedule: 3 months at $49 then $79/month.
    Returns (checkout_url, None) or (None, error).
    """
    _get_stripe_key()
    try:
        # Get or create Stripe customer
        ...
        session = stripe.checkout.Session.create(
            ...
            success_url=f"{BASE_URL}/pro-signup/success?session_id={{CHECKOUT_SESSION_ID}}&token={signup_token}",
            cancel_url=f"{BASE_URL}/pro-signup/cancelled",
            metadata=meta,
        )

        # Store customer_id on subscriber record
        try:
            import sqlite3, pathlib, os
            db_path = os.environ.get("DB_PATH", "/app/data/irs_pilot.db")
            conn = sqlite3.connect(db_path)
            conn.execute(
                "UPDATE pro_subscribers SET stripe_customer_id = ? WHERE id = ?",
...
def handle_webhook(payload: bytes, sig_header: str):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if session.get("payment_status") == "paid":
            ...
            # Pro subscription signups are handled separately (subscriber_id-keyed,
            # not product-keyed) so the welcome email + guide always send from
            # here, even if the customer's browser never reaches /pro-signup/success.
            if checkout_type == "pro_subscription":
                import logging
                logger = logging.getLogger(__name__)
                subscriber_id = session["metadata"].get("subscriber_id", "")
                signup_token  = session["metadata"].get("signup_token", "")
                    ...
                    try:
                        with auth.get_db() as conn:
                            already = conn.execute(
                                "SELECT active FROM pro_subscribers WHERE id = ?",
                                (int(subscriber_id),)
                            ).fetchone()
                            conn.execute(
                                "UPDATE pro_subscribers SET stripe_sub_id = ?, "
                                "stripe_customer_id = ?, active = 1 WHERE id = ?",
                                (stripe_sub_id, customer_id, int(subscriber_id))
                            )

                        # Only send the welcome email once — skip if the success-page
                        # route already handled this session (already active).
                        if not already or not already["active"]:
                            if signup_token:
                                auth.mark_pro_signup_token_used(signup_token)
                                row = auth.get_pro_signup_token(signup_token)
                                if row and row["interest_id"]:
                                    with auth.get_db() as conn:
                                        try:
                                            ...
                                            logger.error(f"Pro signup interest update failed: {e}")
                            ...
                            if sub:
                                ...
                                try:
                                    ...
                                    logger.error(f"Pro welcome email failed (subscriber {subscriber_id}): {e}")
                                try:
                                    ...
                                    logger.error(f"Pro approval/docs email failed (subscriber {subscriber_id}): {e}")
                                try:
                                    email_service.send_partner_marketing_package(sub["email"], is_pro=True)
                                except Exception as e:
                                    logger.error(f"Pro guide/marketing package email failed (subscriber {subscriber_id}): {e}")
                                try:
                                    email_service.notify_new_pro_subscriber(
                                        sub["email"], sub["firm_name"], None
                                    )
                                except Exception as e:
                                    logger.error(f"Tyrone new-subscriber notification failed: {e}")
                    except Exception as e:
                        logger.error(f"Pro subscription webhook activation error (subscriber {subscriber_id}): {e}")

                return True, "pro subscription processed"
