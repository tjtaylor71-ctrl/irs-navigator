...
def generate_video():
    ...
    # Pick a different story angle per video in the batch
    stories = pick_stories(count, exclusions)
    ...
    system_prompt = (
        'You are the video script director for a viral Michael Jackson tribute page. '
        'CONTENT SOURCING RULES - absolute and non-negotiable: '
        '1. Only write about events documented in named interviews, named publications, confirmed video or audio, or verified biographical sources. '
        '2. If no real documented source exists for a story or detail, do not include it. Use a different angle. '
        '3. Never invent dialogue, quotes, or witness accounts. Never attribute words to MJ or anyone else without a named source. '
        '4. Never describe what someone felt, thought, or said privately unless it is documented in a named source. '
        '5. Never fill gaps with plausible-sounding detail. If you do not know a specific fact, do not write it. '
        '6. NEVER frame anything as a "mystery," "lost," "vault," "never released," "leaked," or "hidden" unless that exact framing is independently verifiable - avoid these framings entirely. '
        '7. The script must reference the real source of the story - the named interview, publication, documentary, or person. '
        'CONTENT PREFERENCES: Humanitarian stories, everyday life, genuine documented moments from named sources. '
        'NEVER DEFAULT TO: Wembley Stadium, Thriller recording, Quincy Jones unless specifically requested. '
        'ABSOLUTELY NO EMOJIS in script body. Plain text only. '
        'Respond ONLY with valid JSON. No markdown. No code fences. No preamble.'
    )

    user_prompt = f"""Generate exactly {count} unique Michael Jackson video narration scripts.

{"MANDATORY TOPIC: " + override + "  -  ALL scripts must directly address this specific topic." if override else "Each script MUST be based on a DIFFERENT story angle below  -  do not combine them:" + chr(10) + chr(10).join([f"Script {i+1}: {s['angle']}" for i, s in enumerate(stories)])}
...
- "imagePrompt": A cinematic still image prompt depicting THE ACTUAL SUBJECT of this script's story angle (not a generic MJ shot). For song/album stories, ROTATE scene types - music video recreation in correct costume and setting, dramatic stage performance with crowd, era-appropriate editorial/album-art aesthetic, candid award show or press moment - only use recording studio occasionally. If the story involves family members, collaborators, or other named people, include them in the scene. The scene must directly illustrate what the script is about. Include Michael Jackson's authentic likeness anchored to the correct era with specific physical descriptors. State it is the real Michael Jackson. End with "Optimized for 1080x1350 format."
...
def generate_carousel():
    ...
    try:
        ...
        system_prompt = (
            ...
            'State it is the real Michael Jackson. Anchor likeness to correct era with specific physical descriptors. '
            'Respond ONLY with valid JSON. No markdown. No code fences. No preamble.'
        )
    ...
    <div class="pane" id="pane-carousel">
      <div class="gen-box">
        ...
        <div style="margin-bottom:12px;">
          ...
          <input type="text" id="carousel-override" placeholder="e.g. The making of Billie Jean..." style="width:100%;background:var(--dark2);border:1px solid var(--dark3);border-radius:6px;padding:8px 10px;font-size:13px;color:var(--text);font-family:DM Sans,sans-serif;margin-top:4px;box-sizing:border-box;">
        </div>
...
async function generateCarousel() {
  ...
  try {
    ...
    if (c.topics_used) addToTopicHistory(c.topics_used);

    const goldBorder = '1px solid var(--gold-dim)';
    const dimBorder  = '1px solid var(--dark3)';

    function slideCard(num, label, captionLabel, caption, imgPrompt, borderColor, extraNote) {
      return `<div style="background:var(--dark2);border:${borderColor};border-radius:10px;padding:16px;margin-bottom:12px;">
        ...
        <div style="margin-bottom:10px;">
          ...
          ${extraNote ? `<div style="font-size:10px;color:var(--hint);margin-top:4px;">${extraNote}</div>` : ''}
    ...
    // Slide 1
    html += slideCard(1, 'Slide 1 — Hook', 'Caption (sits below image — longer, descriptive)',
      c.slide1Caption || '', c.slide1ImagePrompt || '', goldBorder,
      'This caption goes below/beside the image, not on top — so it can be fuller.');
    ...
    (c.slides || []).forEach(slide => {
      ...
      html += slideCard(
        ...
        'Overlay Caption (sits ON TOP of image — keep it short)',
    ...
    // First comment
    html += `<div style="background:rgba(201,168,76,.06);border:${goldBorder};border-radius:8px;padding:12px;margin-bottom:8px;">
    ...
    if (c.source) {
      html += `<div style="padding:5px 10px;background:rgba(201,168,76,.06);border-radius:4px;border-left:2px solid var(--gold-dim);"><span style="font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);">Source </span><span style="font-size:11px;color:var(--muted);">${esc(c.source)}</span></div>`;
    }
...
async function runStoryRefresh() {
  ...
  if (totalAdded > 0) {
    ...
    btn.textContent = `✓ ${finalTotal} stories now available`;
