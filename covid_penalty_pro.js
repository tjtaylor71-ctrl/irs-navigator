...
function CovidPenaltyPro() {
  ...
  if (!user.loggedIn) return React.createElement("div", { style:{fontFamily:"'DM Sans',sans-serif",minHeight:"100vh",background:"#f8f6f1"} },
    React.createElement(Nav, { user: null }),
    React.createElement("div", { style:{maxWidth:560,margin:"60px auto",padding:"0 16px"} },
      React.createElement("div", { style:{background:"#fff",border:"1px solid #e8e4dc",borderRadius:12,padding:"36px 32px",textAlign:"center"} },
        React.createElement("div", { style:{fontSize:44,marginBottom:16} }, "🔒"),
        React.createElement("div", { style:{fontWeight:"bold",fontSize:20,color:"#1a2d5a",marginBottom:8} }, "Sign In to Continue"),
        React.createElement("p", { style:{fontSize:14,color:"#555",lineHeight:1.7,marginBottom:24} }, "The Tax Pro COVID Penalty Relief Tool requires an account. Sign in or create a free account to get started."),
        React.createElement("div", { style:{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"} },
          React.createElement("a", { href:"/login?next=/covid-penalty-pro", style:{background:"#1a2d5a",color:"#7ec11f",border:"2px solid #7ec11f",borderRadius:8,padding:"11px 24px",textDecoration:"none",fontWeight:"bold",fontSize:14} }, "Sign In →"),
          React.createElement("a", { href:"/login?mode=register&next=/covid-penalty-pro", style:{background:"transparent",color:"#1a2d5a",border:"1px solid #e8e4dc",borderRadius:8,padding:"11px 24px",textDecoration:"none",fontSize:14} }, "Create Free Account")
        )
      )
    )
  );
  ...
  if (!user.hasCovid) {
    ...
    const isReferral = credits && credits.is_referral;
    const isPro = credits && credits.is_pro;
    const isMember = isReferral || isPro;
    const hasCredits = credits && credits.total > 0;
    const exhausted = hasCredits && credits.remaining === 0;

    let icon, headline, body, actions;

    if (exhausted) {
      icon = "📭";
      headline = "No Credits Remaining";
      body = "You’ve used all your letter credits. Purchase more to continue analyzing transcripts.";
      actions = React.createElement("div", { style:{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"} },
        React.createElement("button", { onClick:function(){setBuyModal(true);}, style:{background:"#1a2d5a",color:"#7ec11f",border:"2px solid #7ec11f",borderRadius:8,padding:"11px 24px",fontWeight:"bold",fontSize:14,cursor:"pointer"} }, "+ Purchase More Credits")
      );
    } else if (isMember && !hasCredits) {
      const memberLabel = isPro ? "Pro Subscriber" : "Referral Member";
      const rate = isPro ? "$50/letter" : "$75/letter";
      icon = "🎟️";
      headline = "Purchase Your First Credits";
      body = "You’re enrolled as a " + memberLabel + " (" + rate + "). Purchase credits to start analyzing client transcripts.";
      actions = React.createElement("div", { style:{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"} },
        React.createElement("button", { onClick:function(){setBuyModal(true);}, style:{background:"#1a2d5a",color:"#7ec11f",border:"2px solid #7ec11f",borderRadius:8,padding:"11px 24px",fontWeight:"bold",fontSize:14,cursor:"pointer"} }, "Purchase Credits →")
      );
    } else {
      icon = "🏦";
      headline = "Join to Get Access";
      body = "To use the Tax Pro COVID Penalty Relief Tool, you need to be a Referral Member (free) or a Pro Subscriber. Choose your path below.";
      actions = React.createElement("div", { style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:4} },
        React.createElement("a", { href:"/referral", style:{background:"#f0fdf4",border:"2px solid #7ec11f",borderRadius:10,padding:"18px 16px",textDecoration:"none",display:"block"} },
          React.createElement("div", { style:{fontSize:22,marginBottom:8} }, "🤝"),
          React.createElement("div", { style:{fontWeight:"bold",fontSize:14,color:"#1a2d5a",marginBottom:4} }, "Referral Member"),
          React.createElement("div", { style:{fontSize:12,color:"#555",lineHeight:1.6,marginBottom:10} }, "Free to join. Earn commissions on referrals. Credits at $75/letter."),
          React.createElement("div", { style:{color:"#7ec11f",fontWeight:"bold",fontSize:13} }, "Join Free →")
        ),
        React.createElement("a", { href:"/taxpro", style:{background:"#f0f7ff",border:"2px solid #1a2d5a",borderRadius:10,padding:"18px 16px",textDecoration:"none",display:"block"} },
          React.createElement("div", { style:{fontSize:22,marginBottom:8} }, "🏢"),
          React.createElement("div", { style:{fontWeight:"bold",fontSize:14,color:"#1a2d5a",marginBottom:4} }, "Pro Subscriber"),
          React.createElement("div", { style:{fontSize:12,color:"#555",lineHeight:1.6,marginBottom:10} }, "$49/month intro. White-label branding. Credits at $50/letter."),
          React.createElement("div", { style:{color:"#1a2d5a",fontWeight:"bold",fontSize:13} }, "Learn More →")
        )
      );
    }
    ...
    return React.createElement("div", { style:{fontFamily:"'DM Sans',sans-serif",minHeight:"100vh",background:"#f8f6f1"} },
      ...
      React.createElement("div", { style:{maxWidth:560,margin:"60px auto",padding:"0 16px"} },
        React.createElement("div", { style:{background:"#fff",border:"1px solid #e8e4dc",borderRadius:12,padding:"36px 32px",textAlign:"center"} },
          React.createElement("div", { style:{fontSize:44,marginBottom:16} }, icon),
          React.createElement("div", { style:{fontWeight:"bold",fontSize:20,color:"#1a2d5a",marginBottom:8} }, headline),
          React.createElement("p", { style:{fontSize:14,color:"#555",lineHeight:1.7,marginBottom:24} }, body),
          actions,
          React.createElement("div", { style:{marginTop:20,paddingTop:16,borderTop:"1px solid #f0ede8"} },
            React.createElement("a", { href:"/demo-covid", style:{color:"#888",fontSize:13,textDecoration:"none"} }, "View the demo first →")
          )
        )
      ),
      buyModal && React.createElement("div", { style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16} },
        React.createElement("div", { style:{background:"#fff",borderRadius:14,padding:"28px 32px",maxWidth:480,width:"100%"} },
          React.createElement("div", { style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16} },
            React.createElement("div", { style:{fontWeight:"bold",fontSize:17,color:"#1a2d5a"} }, "Purchase Credits"),
          ...
          [{tier:"1",label:"1 Letter",price:isPro?"$50":"$75"},{tier:"5",label:"5 Letters",price:isPro?"$250":"$375"},{tier:"10",label:"10 Letters",price:isPro?"$500":"$750"}].map(function(opt) {
            return React.createElement("div", { key:opt.tier, style:{border:"1px solid #e8e4dc",borderRadius:8,padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"} },
              React.createElement("div", { style:{fontWeight:"bold",fontSize:14,color:"#1a2d5a"} }, opt.label),
              React.createElement("div", { style:{display:"flex",alignItems:"center",gap:12} },
                React.createElement("div", { style:{fontWeight:"bold",fontSize:16,color:"#1a2d5a"} }, opt.price),
                React.createElement("button", { onClick:function(){buyCredits(opt.tier);}, disabled:buying, style:{background:"#7ec11f",color:"#1a2d5a",border:"none",borderRadius:7,padding:"8px 16px",fontWeight:"bold",fontSize:13,cursor:"pointer"} }, buying?"...":"Buy →")
              )
            );
          })
        )
