...
function BookkeepingTool() {
  ...
  const [matchModal, setMatchModal] = useState(null); // {desc, category, matches, partial}
  const [newCatName, setNewCatName] = useState("");
  const [newCatQbo, setNewCatQbo] = useState("");
  ...
  async function loadSessions() {
    try {
      const r = await fetch("/api/admin/bookkeeping/sessions", { credentials: "include" });
  ...
  async function saveSession() {
    setSaving(true);
    try {
      const r = await fetch("/api/admin/bookkeeping/sessions", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: sessionId, client_name: rClient || "Unnamed Client", period: rPeriod, notes: rNote, transactions: txns })
      });
      const d = await r.json();
      if (d.ok) { setSessionId(d.id); showToast("Session saved"); }
  ...
  async function loadSession(sid) {
    try {
      const r = await fetch("/api/admin/bookkeeping/sessions/" + sid, { credentials: "include" });
      const d = await r.json();
      setTxns(d.transactions || []);
      setRClient(d.client_name || "");
      setRPeriod(d.period || "");
      setRNote(d.notes || "");
      setSessionId(d.id);
  ...
  async function loadCategories() {
    try {
      const r = await fetch("/api/admin/bookkeeping/categories", { credentials: "include" });
      const d = await r.json();
  ...
  async function loadRules() {
    try {
      const r = await fetch("/api/admin/bookkeeping/rules", { credentials: "include" });
      const d = await r.json();
      setRules(d.rules || []);
    } catch(e) {}
  }

  async function saveRule(keyword, matchType, category) {
    await fetch("/api/admin/bookkeeping/rules", {
      method: "POST", credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ keyword, match_type: matchType, category })
    });
    loadRules();
  }

  async function deleteRule(rid) {
    await fetch("/api/admin/bookkeeping/rules/" + rid, { method: "DELETE", credentials: "include" });
    loadRules();
  }

  async function createCategory(name, qbo) {
    if (!name.trim()) return;
    await fetch("/api/admin/bookkeeping/categories", {
      method: "POST", credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: name.trim(), qbo_account: qbo.trim() })
    });
    loadCategories();
    setNewCatName(""); setNewCatQbo("");
  }

  async function deleteCategory(cid) {
    await fetch("/api/admin/bookkeeping/categories/" + cid, { method: "DELETE", credentials: "include" });
    loadCategories();
  }

  function handleCatChange(id, newCat) {
    if (newCat === "__new__") { setShowCatMgr(true); return; }
    const t = txns.find(x => x.id === id);
    if (!t) return;
    const oldCat = t.category;
    if (newCat === oldCat) return;

    // Find exact matches (same full description)
    const exactMatches = txns.filter(x => x.id !== id && x.description === t.description && x.category !== newCat);
    // Find partial keyword matches (first meaningful word of description)
    const keyword = t.description.split(" ").slice(0, 4).join(" ");
    const partialMatches = txns.filter(x => x.id !== id && x.description !== t.description &&
      x.description.includes(keyword.split(" ")[2] || keyword.split(" ")[1] || keyword.split(" ")[0]) &&
      x.category !== newCat);

    // Apply to this transaction first
    updateCat(id, newCat);

    if (exactMatches.length > 0 || partialMatches.length > 0) {
      setMatchModal({ id, desc: t.description, category: newCat, keyword, exactMatches, partialMatches });
    }
  }

  function applyMatchModal(applyExact, applyPartial) {
    if (!matchModal) return;
    const { exactMatches, partialMatches, category, desc, keyword } = matchModal;
    if (applyExact) {
      exactMatches.forEach(t => updateCat(t.id, category));
      saveRule(desc, "exact", category);
    }
    if (applyPartial) {
      partialMatches.forEach(t => updateCat(t.id, category));
      const kw = keyword.split(" ").slice(0, 3).join(" ");
      saveRule(kw, "keyword", category);
    }
    setMatchModal(null);
  }

  async function deleteSession(sid) {
    await fetch("/api/admin/bookkeeping/sessions/" + sid, { method: "DELETE", credentials: "include" });
    loadSessions();
  ...
  useEffect(() => { loadSessions(); loadCategories(); loadRules(); }, []);
  ...
  async function processFiles() {
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      try {
        ...
        // Submit job
        const submitResp = await fetch("/api/admin/bookkeeping/extract", {
          method: "POST", credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ pdf_base64: b64, filename: item.file.name })
        });
        const submitData = await submitResp.json();
        if (!submitResp.ok) throw new Error(submitData.error || `Server error ${submitResp.status}`);
        ...
        for (let attempt = 0; attempt < 200; attempt++) {
          ...
          const pollResp = await fetch(`/api/admin/bookkeeping/job/${jobId}`, { credentials: "include" });
          const pollData = await pollResp.json();
          if (pollData.status === "done") { jobResult = pollData; break; }
          if (pollData.status === "error") throw new Error(pollData.error || "Extraction failed");
          setProgress({ pct: Math.round(((i + 0.5) / pending.length) * 90), text: `${item.file.name}: ${pollData.progress || "Processing..."}` });
        }
        if (!jobResult) throw new Error("Timed out waiting for extraction");

        const mapped = (jobResult.transactions || []).map((t, idx) => ({
    ...
    if (collected.length) {
      ...
      setTab("transactions");
      try {
        const r2 = await fetch("/api/admin/bookkeeping/sessions", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sessionId, client_name: rClient || "Unnamed Client", period: rPeriod, notes: rNote, transactions: collected }) });
        const d2 = await r2.json();
        if (d2.ok) setSessionId(d2.id);
      } catch(e) {}
    }
  }
  ...
  return React.createElement("div", { style: S.wrap },
    ...
    tab === "upload" && React.createElement("div", null,
      ...
      /* session bar */
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 14, flexWrap: "wrap", gap: 8 } },
        React.createElement("div", { style: { display: "flex", gap: 8 } },
          React.createElement("button", { style: S.btn(false), onClick: () => { setShowSessions(v => !v); loadSessions(); } }, "Load Session" + (sessions.length ? " (" + sessions.length + ")" : "")),
          txns.length > 0 && React.createElement("button", { style: { ...S.btn(false), opacity: saving ? 0.5 : 1 }, disabled: saving, onClick: saveSession }, saving ? "Saving..." : "Save Session")
        ),
        files.length > 0 && React.createElement("div", { style: { display: "flex", gap: 8 } },
          React.createElement("button", { style: S.btn(false), onClick: () => setFiles([]) }, "Clear files"),
      ...
      /* session list */
      showSessions && React.createElement("div", { style: { marginTop: 10, border: "1px solid #e8e4dc", borderRadius: 12, overflow: "hidden" } },
        React.createElement("div", { style: { background: "#1a2d5a", color: "#fff", padding: "10px 16px", fontWeight: "bold", fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" } },
          "Saved Sessions",
          React.createElement("button", { style: { background: "none", border: "none", color: "#7ec11f", cursor: "pointer", fontSize: 20, lineHeight: 1 }, onClick: () => setShowSessions(false) }, "x")
        ),
        sessions.length === 0
          ? React.createElement("div", { style: { padding: "20px", textAlign: "center", color: "#aaa", fontSize: 13 } }, "No saved sessions yet")
          : sessions.map(s =>
            React.createElement("div", { key: s.id, style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: "1px solid #f0ede8", background: sessionId === s.id ? "#f0f7e8" : "#fff" } },
              React.createElement("div", { style: { flex: 1 } },
                React.createElement("div", { style: { fontWeight: "bold", fontSize: 13, color: "#1a2d5a" } }, s.client_name),
                React.createElement("div", { style: { fontSize: 11, color: "#aaa", marginTop: 2 } }, [s.period, (s.tx_count || 0) + " transactions", (s.updated_at || "").slice(0,10)].filter(Boolean).join(" - "))
              ),
              React.createElement("button", { style: { ...S.btn(true), padding: "4px 12px", fontSize: 12 }, onClick: () => loadSession(s.id) }, "Load"),
              React.createElement("button", { style: { background: "#fce4ec", color: "#c62828", border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer", fontWeight: "bold" }, onClick: () => { if(window.confirm("Delete this session?")) deleteSession(s.id); } }, "Del")
            )
          )
      )
    ),
    ...
    tab === "transactions" && React.createElement("div", null,
      ...
      React.createElement("div", { style: { display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" } },
        ...
        React.createElement("button", { style: { ...S.btn(false) }, onClick: () => setShowCatMgr(true) }, "Manage Categories"),
                ...
                React.createElement("tr", { key: t.id },
                  ...
                  React.createElement("td", { style: S.td },
                    React.createElement("select", {
                      ...
                      [...BK_CATS, ...customCats.map(c => c.name).filter(n => !BK_CATS.includes(n))].map(c =>
                        React.createElement("option", { key: c, value: c }, c)
                      ),
                      React.createElement("option", { value: "__new__", style: { fontStyle: "italic", color: "#888" } }, "+ Add custom category...")
                    )
                  ),
    ...
    /* ── Auto-match modal ── */
    matchModal && React.createElement("div", {
      style: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" }
    },
      React.createElement("div", { style: { background: "#fff", borderRadius: 14, padding: "24px", width: 500, maxWidth: "92vw", border: "1px solid #e8e4dc" } },
        React.createElement("div", { style: { fontWeight: "bold", fontSize: 16, color: "#1a2d5a", marginBottom: 6 } }, "Auto-match other transactions?"),
        React.createElement("div", { style: { fontSize: 13, color: "#666", marginBottom: 16 } },
          'You categorized "' + matchModal.desc.slice(0, 50) + (matchModal.desc.length > 50 ? "..." : "") + '" as ',
          React.createElement("strong", null, matchModal.category)
        ),
        matchModal.exactMatches.length > 0 && React.createElement("div", { style: { background: "#f0f7e8", border: "1px solid #c8e6c9", borderRadius: 8, padding: "12px 14px", marginBottom: 12 } },
          React.createElement("div", { style: { fontWeight: "bold", fontSize: 13, color: "#1b5e20", marginBottom: 6 } },
            matchModal.exactMatches.length + " other transaction" + (matchModal.exactMatches.length > 1 ? "s" : "") + " with the exact same description"
          ),
          matchModal.exactMatches.slice(0, 3).map((t, i) =>
            React.createElement("div", { key: i, style: { fontSize: 12, color: "#555", marginBottom: 2 } },
              t.date + " | " + (t.amount >= 0 ? "+" : "") + "$" + Math.abs(t.amount).toFixed(2)
            )
          ),
          matchModal.exactMatches.length > 3 && React.createElement("div", { style: { fontSize: 11, color: "#888" } }, "...and " + (matchModal.exactMatches.length - 3) + " more")
        ),
        matchModal.partialMatches.length > 0 && React.createElement("div", { style: { background: "#fff8e1", border: "1px solid #ffe082", borderRadius: 8, padding: "12px 14px", marginBottom: 12 } },
          React.createElement("div", { style: { fontWeight: "bold", fontSize: 13, color: "#e65100", marginBottom: 6 } },
            matchModal.partialMatches.length + " similar transaction" + (matchModal.partialMatches.length > 1 ? "s" : "") + " (keyword match)"
          ),
          matchModal.partialMatches.slice(0, 3).map((t, i) =>
            React.createElement("div", { key: i, style: { fontSize: 12, color: "#555", marginBottom: 2 } },
              t.date + " | " + t.description.slice(0, 40) + " | $" + Math.abs(t.amount).toFixed(2)
            )
          ),
          matchModal.partialMatches.length > 3 && React.createElement("div", { style: { fontSize: 11, color: "#888" } }, "...and " + (matchModal.partialMatches.length - 3) + " more")
        ),
        React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 } },
          matchModal.exactMatches.length > 0 && matchModal.partialMatches.length > 0 && React.createElement("button", {
            style: { ...S.btn(true), flex: 1 },
            onClick: () => applyMatchModal(true, true)
          }, "Apply to all (" + (matchModal.exactMatches.length + matchModal.partialMatches.length) + ")"),
          matchModal.exactMatches.length > 0 && React.createElement("button", {
            style: { ...S.btn(matchModal.partialMatches.length === 0), flex: 1 },
            onClick: () => applyMatchModal(true, false)
          }, "Exact matches only (" + matchModal.exactMatches.length + ")"),
          matchModal.partialMatches.length > 0 && React.createElement("button", {
            style: { ...S.btn(false), flex: 1 },
            onClick: () => applyMatchModal(false, true)
          }, "Similar only (" + matchModal.partialMatches.length + ")"),
          React.createElement("button", { style: S.btn(false), onClick: () => setMatchModal(null) }, "Skip")
        )
      )
    ),

    /* ── Category manager modal ── */
    showCatMgr && React.createElement("div", {
      style: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" },
      onClick: e => { if (e.target === e.currentTarget) setShowCatMgr(false); }
    },
      React.createElement("div", { style: { background: "#fff", borderRadius: 14, padding: "24px", width: 480, maxWidth: "92vw", border: "1px solid #e8e4dc", maxHeight: "80vh", overflowY: "auto" } },
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
          React.createElement("div", { style: { fontWeight: "bold", fontSize: 16, color: "#1a2d5a" } }, "Custom Categories"),
          React.createElement("button", { style: { background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#888" }, onClick: () => setShowCatMgr(false) }, "x")
        ),
        React.createElement("div", { style: { display: "flex", gap: 8, marginBottom: 12 } },
          React.createElement("input", { style: { ...S.input, flex: 2 }, placeholder: "Category name (e.g. Owner Draw)", value: newCatName, onChange: e => setNewCatName(e.target.value) }),
          React.createElement("input", { style: { ...S.input, flex: 2 }, placeholder: "QBO account name (optional)", value: newCatQbo, onChange: e => setNewCatQbo(e.target.value) }),
          React.createElement("button", { style: S.btn(true), onClick: () => createCategory(newCatName, newCatQbo) }, "Add")
        ),
        customCats.length === 0
          ? React.createElement("div", { style: { color: "#aaa", fontSize: 13, textAlign: "center", padding: "16px" } }, "No custom categories yet")
          : customCats.map(c =>
            React.createElement("div", { key: c.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid #f0ede8" } },
              React.createElement("div", { style: { flex: 1 } },
                React.createElement("div", { style: { fontWeight: "bold", fontSize: 13, color: "#1a2d5a" } }, c.name),
                c.qbo_account && React.createElement("div", { style: { fontSize: 11, color: "#888" } }, "QBO: " + c.qbo_account)
              ),
              React.createElement("button", { style: { background: "#fce4ec", color: "#c62828", border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer", fontWeight: "bold" }, onClick: () => deleteCategory(c.id) }, "Del")
            )
          ),
        rules.length > 0 && React.createElement("div", { style: { marginTop: 20 } },
          React.createElement("div", { style: { fontWeight: "bold", fontSize: 14, color: "#1a2d5a", marginBottom: 10 } }, "Auto-match Rules"),
          rules.map(r =>
            React.createElement("div", { key: r.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #f0ede8" } },
              React.createElement("div", { style: { flex: 1 } },
                React.createElement("span", { style: { fontSize: 11, background: r.match_type === "exact" ? "#e8f5e9" : "#fff8e1", color: r.match_type === "exact" ? "#2e7d32" : "#e65100", padding: "2px 6px", borderRadius: 10, marginRight: 6, fontWeight: "bold" } }, r.match_type),
                React.createElement("span", { style: { fontSize: 13 } }, '"' + r.keyword.slice(0, 35) + '"'),
                React.createElement("span", { style: { fontSize: 12, color: "#888", marginLeft: 6 } }, "-> " + r.category)
              ),
              React.createElement("button", { style: { background: "#fce4ec", color: "#c62828", border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer", fontWeight: "bold" }, onClick: () => deleteRule(r.id) }, "Del")
            )
          )
        )
      )
