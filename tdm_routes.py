...
function renderVideos(type) {
  ...
  arr.forEach((v, i) => {
    ...
    const imgs = v.images || [];
    const imgBlock = imgs.length ? (function() {
    var out = '<div style="margin:0 0 12px;padding:12px 14px;background:var(--dark2);border-radius:8px;">';
    out += '<div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:10px;">' + imgs.length + ' IMAGE PROMPTS (9:16 for Seedance/Kling)</div>';
    imgs.forEach(function(img) {
      out += '<div style="margin-bottom:12px;">';
      out += '<div style="font-size:9px;font-weight:700;color:var(--gold);margin-bottom:3px;">IMAGE ' + img.num + '</div>';
      out += '<div style="font-size:12px;color:var(--muted);line-height:1.5;font-style:italic;">' + esc(img.prompt) + '</div>';
      out += '<button class="tdm-copy-prompt mbtn" data-prompt="' + encodeURIComponent(img.prompt) + '" style="margin-top:4px;font-size:10px;">Copy</button>';
      out += '</div>';
    });
    var allText = imgs.map(function(img){ return 'Image ' + img.num + ':\n' + img.prompt; }).join('\n\n');
    out += '<button class="tdm-copy-all mbtn" data-prompts="' + encodeURIComponent(allText) + '" style="margin-top:4px;border:1px solid var(--gold-dim);color:var(--gold);font-size:10px;">Copy All ' + imgs.length + ' Prompts</button>';
    out += '</div>';
    return out;
  })() : '';
