...
async function genSingleImage(idx) {
  ...
  try {
    ...
    imgDiv.innerHTML = \'<div class="drop-zone" id="dz-\' + idx + \'" onclick="triggerUpload(\' + idx + \')" ondragover="dzOver(event,\' + idx + \')" ondragleave="dzLeave(\' + idx + \')" ondrop="dzDrop(event,\' + idx + \')"><span style="font-size:18px">⚠</span><span>Error — retry or upload</span></div>\';
...
function clearAll() {
  S.posts = [];
  document.getElementById(\'posts-container\').innerHTML = \'<div class="empty"><span class="empty-icon">👑</span><h3>Awaiting Orders</h3><p>Enter a theme above and click Generate to brief the crew.</p></div>\';
  document.getElementById(\'batch-bar\').classList.remove(\'show\');
  document.getElementById(\'img-progress\').style.display = \'none\';
  document.getElementById(\'sched-container\').innerHTML = \'<div class="empty"><span class="empty-icon">📅</span><h3>No Posts Yet</h3><p>Generate content first.</p></div>\';
  updateStats();
}
