...
<div class="layout">
  ...
  <div class="main">
    ...
    <div class="pane" id="pane-shorts">
      <div class="theme-box">
        <div class="day-name-d" id="short-day-d">Loading...</div>
        <p>Generates 3 x 30-second video scripts with 5 image prompts with captions and scene image prompts.</p>
        <div class="btn-row">
    ...
    <div class="pane" id="pane-longs">
      <div class="theme-box">
        <div class="day-name-d" id="long-day-d">Loading...</div>
        <p>Generates 3 x 60-second video scripts with 10 image prompts with captions and scene image prompts.</p>
        <div class="btn-row">
...
// ?? RENDER VIDEO CARDS ????????????????????????????????????
function buildTdmImgBlock(container, imgs) {
  ...
  imgs.forEach(function(img) {
    ...
    text.style.cssText = "font-size:12px;color:var(--muted);line-height:1.5;font-style:italic;margin-bottom:4px;";
    ...
    wrap.appendChild(label); wrap.appendChild(text); wrap.appendChild(btn);
    container.appendChild(wrap);
  });
  var allText = imgs.map(function(img) { return "Image " + img.num + ": " + (img.prompt || ""); }).join("\n---\n");
  ...
  copyAll.addEventListener("click", function() {
    if (navigator.clipboard) { navigator.clipboard.writeText(allText).then(function() { copyAll.textContent = "Copied!"; setTimeout(function() { copyAll.textContent = "Copy All " + imgs.length + " Prompts"; }, 1500); }); }
  });
