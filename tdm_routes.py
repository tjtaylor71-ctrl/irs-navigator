...
1. "imagePrompt": Photorealistic cinematic 4:5 image. Yorkie with steel-blue and tan fur. Specific situation, tiny outfit, bold gold text overlay with headline, Tiny Dog Mafia division subtitle. Warm cinematic lighting, shallow depth of field.
2. "caption": HOOK line. 3-4 short scene lines. PAYOFF with 2-3 emojis. Closing deadpan line treating {dog_name} as authority. CTA ({cta_map[cta]}). 5-6 hashtags. Tone: {tone_map[tone]}
3. "firstComment": 1-2 sentence engaging follow-up. No hashtags.
Also: "headline" (2-6 word ALL CAPS), "division" (fake TDM division name)
...
Rules: {rules}
For each video:
1. "videoScript": {note} Pure narration only.
2. "captionAndHashtags": Hook line. 2-3 comedic context lines. CTA. 5-6 hashtags.
3. "firstComment": 1-2 sentences adding to the comedy.
...
document.getElementById('copy-all-captions-btn').addEventListener('click', () => {
  const txt = S.imagePosts.map((p, i) => 'POST ' + (i+1) + ':\n' + p.caption).join('\n\n---\n\n');
...
function buildCardHTML(type, idx, item, time, typeLabel, typeCls, s1text, s2text, s3text, tabs, metaExtra) {
  ...
  const tabsHTML = tabs.map((tab, ti) => {
    const paneId = type + '-s' + (ti+1) + '-' + idx;
    const allPanes = tabs.map((_, tii) => type + '-s' + (tii+1) + '-' + idx).join(',');
    return '<div class="sec-tab ' + (ti===0?'on':'') + '" data-action="sectab" data-panes="' + allPanes + '" data-show="' + paneId + '">' + tab.label + '</div>';
  }).join('');

  const panesHTML = tabs.map((tab, ti) => {
    const paneId = type + '-s' + (ti+1) + '-' + idx;
    const text = ti===0 ? s1text : ti===1 ? s2text : s3text;
