...
function renderVideos(type) {
  ...
  arr.forEach((v, i) => {
    ...
    if (vimgs.length) {
      ...
      var imgHTML = '<div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:10px;">' + vimgs.length + ' IMAGE PROMPTS - 9:16 for Seedance/Kling</div>';
      vimgs.forEach(function(img) {
        imgHTML += '<div style="margin-bottom:10px;">';
        imgHTML += '<div style="font-size:9px;font-weight:700;color:var(--gold);margin-bottom:3px;">IMAGE ' + img.num + '</div>';
        imgHTML += '<div style="font-size:12px;color:var(--muted);line-height:1.5;font-style:italic;margin-bottom:4px;">' + esc(img.prompt || '') + '</div>';
        imgHTML += '<button class="tdm-copy-prompt mbtn" data-prompt="' + encodeURIComponent(img.prompt || '') + '" style="font-size:10px;">Copy</button>';
        imgHTML += '</div>';
      });
      var allTxt = vimgs.map(function(img){ return 'Image ' + img.num + ':\n' + (img.prompt||''); }).join('\n\n');
      imgHTML += '<button class="tdm-copy-all mbtn" data-prompts="' + encodeURIComponent(allTxt) + '" style="border:1px solid var(--gold-dim);color:var(--gold);font-size:10px;">Copy All ' + vimgs.length + ' Prompts</button>';
