...
Draw from these content pillars — spread posts across as many different pillars as possible:
{chr(10).join([f"{i+1}. {p}" for i, p in enumerate(selected[:count])])}

STRICT ANTI-REPETITION RULES:
- Every post MUST have a completely different situation, setting, room, activity, and scenario
- Never repeat the same activity (only one post about laundry, one about cooking, etc.)
- Vary the location: kitchen, living room, bedroom, backyard, car, front door, bathroom, office, etc.
- Vary the comedic angle: supervising, claiming territory, demanding attention, causing chaos, refusing orders

CAPTION STYLE — "your Yorkie" voice, short punchy lines building an observation:
- Example: "Nobody invited your Yorkie to host the barbecue. Nobody handed them responsibilities. Yet somehow... They've greeted every guest. At this point, your Yorkie is networking. 🐾😂"
- 3 full paragraphs: Para 1 = scroll-stopping hook with emojis, Para 2 = build the scene/story, Para 3 = CTA
...
FIRST COMMENT: 1-2 sentences, warm owner-to-owner voice. Funny extra observation + "Tag a friend who needs to see this." Never hashtags.
