  ...
  <div class="sidebar">
    ...
    <div class="s-section">
      ...
      <div class="s-field"><label class="s-label">Image posts per day</label>
        <select id="post-count"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="10" selected>10</option><option value="11">11</option><option value="12">12</option><option value="13">13</option><option value="14">14</option><option value="15">15</option><option value="16">16</option><option value="17">17</option><option value="18">18</option><option value="19">19</option><option value="20">20</option></select>
      </div>
  ...
  <div class="main">
    ...
    <div class="pane on" id="pane-images">
      <div class="theme-box">
        ...
        <p>Enter a theme or leave blank to use the daily theme. <strong style="color:var(--gold);">Tip: Generate 10 at a time for best results.</strong></p>
        <div class="theme-row">
...
TDM_STUDIO_HTML = '''  Tiny Dog Mafia Content Studio
"""
import os, json, base64, requests, random
from flask import Blueprint, request, jsonify, Response
...
function renderVideos(type) {
  ...
  arr.forEach((v, i) => {
    ...
    const card = document.createElement('div');
    card.className = 'content-card'; card.id = type + '-card-' + i;
    const baseCard = buildCardHTML(type, i, v, t, typeLabel, typeCls,
      v.videoScript, v.captionAndHashtags, v.firstComment,
    ...
    const imgs = v.images || [];
    const imgBlock = imgs.length ? '<div style="margin:0 0 12px;padding:12px 14px;background:var(--dark2);border-radius:8px;">' +
      '<div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:10px;">' + imgs.length + ' IMAGE PROMPTS (9:16 for Seedance/Kling)</div>' +
      imgs.map(function(img) { return '<div style="margin-bottom:12px;">' +
        '<div style="font-size:9px;font-weight:700;color:var(--gold);margin-bottom:3px;">IMAGE ' + img.num + '</div>' +
        '<div style="font-size:12px;color:var(--muted);line-height:1.5;font-style:italic;">' + esc(img.prompt) + '</div>' +
        '<button onclick="tdmQuickCopy(this,'' + esc(img.prompt).replace(/'/g,"\'") + '')" style="margin-top:4px;background:var(--dark3);border:none;border-radius:4px;padding:3px 8px;font-size:10px;color:var(--muted);cursor:pointer;font-family:DM Sans,sans-serif;">Copy</button>' +
        '</div>'; }).join('') +
      '<button onclick="tdmQuickCopy(this,'' + esc(imgs.map(function(img){return 'Image '+img.num+':\n'+img.prompt;}).join('\n\n')).replace(/'/g,"\'") + '')" style="margin-top:4px;background:var(--dark3);border:1px solid var(--gold-dim);border-radius:4px;padding:5px 12px;font-size:10px;color:var(--gold);cursor:pointer;font-family:DM Sans,sans-serif;">Copy All ' + imgs.length + ' Prompts</button>' +
      '</div>' : '';
    card.innerHTML = baseCard.replace('</div></div>', imgBlock + '</div></div>');
    list.appendChild(card);
  });
  container.appendChild(list);
}
...
function tdmQuickCopy(btn, text) {
