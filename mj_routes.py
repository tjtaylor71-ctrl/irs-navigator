...
function buildCardHTML(type, idx, item, time, typeLabel, typeCls) {
  ...
  if (type === 'img') {
    ...
    // Build image prompts string
    const imgs = (item.images || []);
    const imgPromptsText = imgs.map(img => 'Image ' + img.num + ':\n' + img.prompt).join('\n\n');
    const cardHTML = buildCard(type, idx, item, time, typeLabel, typeCls,
      item.videoScript, item.captionAndHashtags, item.firstComment, 'Narration Script', 'Caption & Hashtags');
    // Inject image prompts section after the card's tab content
    const imgSection = imgs.length ? `<div style="margin:0 0 12px;padding:12px 16px;background:var(--dark2);border-radius:8px;">
      <div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:10px;">
        ${imgs.length} IMAGE PROMPTS (9:16 for Seedance/Kling)
      </div>
      ${imgs.map(img => `<div style="margin-bottom:12px;">
        <div style="font-size:9px;font-weight:700;color:var(--gold);margin-bottom:3px;">IMAGE ${img.num}</div>
        <div style="font-size:12px;color:var(--muted);line-height:1.6;font-style:italic;">${esc(img.prompt)}</div>
        <button onclick="quickCopy(this,'${esc(img.prompt)}')" style="margin-top:4px;background:var(--dark3);border:none;border-radius:4px;padding:3px 8px;font-size:10px;color:var(--muted);cursor:pointer;font-family:DM Sans,sans-serif;">Copy</button>
      </div>`).join('')}
      <button onclick="quickCopy(this,'${esc(imgPromptsText)}')" style="margin-top:4px;background:var(--dark3);border:1px solid var(--gold-dim);border-radius:4px;padding:5px 12px;font-size:10px;color:var(--gold);cursor:pointer;font-family:DM Sans,sans-serif;">Copy All ${imgs.length} Prompts</button>
    </div>` : '';
    return cardHTML.replace('</div></div>', imgSection + '</div></div>');
  }
}
  ...
  S.carousels.forEach((c, ci) => {
    ...
    out += `<div style="border:1px solid var(--dark3);border-radius:12px;padding:16px;margin-bottom:20px;background:var(--dark1);">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        ...
        <button onclick="deleteCarousel(${ci})" style="background:none;border:1px solid var(--dark3);border-radius:4px;padding:3px 8px;font-size:10px;color:var(--muted);cursor:pointer;font-family:DM Sans,sans-serif;">✕ Remove</button>
      </div>`;
