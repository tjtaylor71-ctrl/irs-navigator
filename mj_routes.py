...
# Weighted content system - audience-preferred topics appear more often
# HIGH weight (4x) = what this audience loves most
HIGH_WEIGHT_PILLARS = [
    "Michael Jackson visiting sick children in hospitals - documented visits, interactions with patients, the personal meaning behind these acts of kindness",
    "Michael Jackson in everyday life - documented moments shopping, visiting theme parks, at home, traveling, relaxing - NOT performing, NOT in signature stage outfits",
    "Michael Jackson as a father and family man - documented moments with his children, protective nature, family gatherings",
    "Michael Jackson's Heal the World Foundation - specific documented programs, children helped, confirmed impact and fundraising events",
    "We Are the World January 28 1985 - the documented creation, who was there, confirmed stories from named participants",
    "Michael Jackson's personal charitable donations - confirmed large gifts to named organizations, documented causes he funded personally",
    "Michael Jackson meeting fans - confirmed stories of him stopping for fans, surprise visits, documented encounters from named sources",
    "Michael Jackson's personal fashion OFF stage - casual documented style, street wear, sunglasses, his documented personal fashion evolution beyond the stage costumes",
    "Michael Jackson's dedication and work ethic - documented rehearsal intensity, confirmed stories from named dancers and crew",
    "Michael Jackson's friendship with Elizabeth Taylor, Marlon Brando, Macaulay Culkin - confirmed documented stories from named sources",
]

# MEDIUM weight (2x) = good content, use regularly
MEDIUM_WEIGHT_PILLARS = [
    "Michael Jackson's Off The Wall era 1979 - tuxedos, bow ties, the sophisticated transition from child star to adult artist, documented photoshoots",
    "Michael Jackson's Jackson 5 era personal life - the young MJ behind the scenes, documented stories from the family",
    "Michael Jackson's Invincible era 2001 - his later career style, documented public appearances",
    "Michael Jackson's Super Bowl XXVII halftime 1993 - the silence opening, confirmed viewership records, specific documented moments",
    "Michael Jackson's Motown 25 May 16 1983 - the moonwalk debut, documented aftermath, confirmed reactions from named witnesses",
    "Michael Jackson's influence on named modern artists - documented quotes from Bruno Mars, Usher, Justin Timberlake, Beyonce",
    "The making of specific music videos - documented production stories for Black or White, Beat It, Smooth Criminal, Remember the Time",
    "Michael Jackson's record-breaking chart achievements - specific Billboard records with dates, verified Guinness records with numbers",
    "Michael Jackson's documented speeches and interviews - specific quotes from Rolling Stone, Ebony, Oprah, Martin Bashir with full context",
    "Michael Jackson's studio sessions with non-Quincy collaborators - Teddy Riley, Bill Bottrell, Rodney Jerkins, documented recording stories",
]

# LOW weight (1x) = valid content but use sparingly
LOW_WEIGHT_PILLARS = [
    "Thriller album recording process - use occasionally, focus on lesser-known aspects like Rod Temperton writing Thriller, not just Quincy Jones",
    "Michael Jackson and Quincy Jones collaboration - use sparingly, focus on specific documented moments not covered recently",
    "Michael Jackson's Bad era - documented stories beyond the obvious hits",
    "Michael Jackson's Dangerous era - Teddy Riley, the new sound, documented production stories",
    "Michael Jackson's HIStory era - the comeback narrative, documented world tour moments beyond Wembley",
    "Michael Jackson's concert tours - specific documented moments from tours other than Wembley, specific setlist choices, confirmed backstage stories",
    "Michael Jackson fan community - documented fan events, the global community, confirmed fan interactions",
    "Michael Jackson's Wembley Stadium performances - use very sparingly, only when other topics are exhausted",
]

import random as _random

def get_weighted_pillar():
    pool = (HIGH_WEIGHT_PILLARS * 4) + (MEDIUM_WEIGHT_PILLARS * 2) + (LOW_WEIGHT_PILLARS * 1)
    return _random.choice(pool)

def get_weighted_pillars(count, exclusions=None):
    pool = (HIGH_WEIGHT_PILLARS * 4) + (MEDIUM_WEIGHT_PILLARS * 2) + (LOW_WEIGHT_PILLARS * 1)
    exclusions = exclusions or []
    # Filter out pillars that mention excluded topics
    if exclusions:
        excl_lower = [e.lower() for e in exclusions]
        pool = [p for p in pool if not any(any(word in p.lower() for word in e.split()[:3]) for e in excl_lower)]
        if not pool:  # fallback if everything filtered
            pool = HIGH_WEIGHT_PILLARS + MEDIUM_WEIGHT_PILLARS
    
    result = []
    seen = set()
    attempts = 0
    while len(result) < count and attempts < 200:
        pick = _random.choice(pool)
        # Use first 5 words as dedup key
        key = ' '.join(pick.split()[:5])
        if key not in seen:
            seen.add(key)
            result.append(pick)
        attempts += 1
    ...
    while len(result) < count:
        result.append(_random.choice(HIGH_WEIGHT_PILLARS))
    
    return result

# Keep PILLARS as the combined list for any code that references it
PILLARS = HIGH_WEIGHT_PILLARS + MEDIUM_WEIGHT_PILLARS + LOW_WEIGHT_PILLARS
...
{chr(10).join([f"{i+1}. {p}" for i, p in enumerate(selected_pillars[:count])]) if not override else ""}
...
def generate_banner():
    ...
    pillar = override if override else get_weighted_pillar()
...
def schedule_buffer():
    ...
    try:
        ...
        return jsonify({'ok':False,'error':str(e)}), 500
user_prompt = f"""Generate exactly {count} unique Michael Jackson {video_type}-form video scripts.

{"MANDATORY TOPIC: " + override + "  -  ALL scripts must directly address this specific topic." if override else "Content pillar: " + pillar}
{chr(10) + "TOPICS TO COMPLETELY AVOID  -  used recently:" + chr(10) + chr(10).join(["- " + e for e in exclusions]) if exclusions else ""}

Rules: {duration_rules}

VIDEO SCRIPT FORMAT:
- {word_guide}
- Open with a powerful hook in the very first line
- Tell a true, emotionally resonant story using ONLY documented, verifiable facts
- Short punchy lines with line breaks for pacing
- End with a clear CTA: ask viewers to drop a comment and follow
- ABSOLUTELY NO EMOJIS in the script body  -  plain text only
- NO shot descriptions, camera directions, or visual notes
- NO invented quotes, private moments, or fabricated details

IMAGE PROMPT: Cinematic still for key scene. Michael Jackson authentic likeness anchored to correct era with specific physical descriptors. State it is the real Michael Jackson. Scene type matching the content (NOT a concert default). End with "Optimized for 1080x1350 format."

CAPTION: Exactly 3 full paragraphs. Para 1: scroll-stopping hook with 2-3 emojis. Para 2: story context with documented facts. Para 3: CTA with 2 emojis. Then exactly 5 hashtags.

FIRST COMMENT: 1-2 sentences, warm fan-to-fan voice, documented verifiable fact + "Tag a friend" CTA. Never hashtags.

For each script include "pillar" (specific 2-3 word topic label) and "topics_used" (comma-separated specific topics referenced)

JSON: {{"videos":[{{"videoScript":"...","imagePrompt":"...","captionAndHashtags":"...","firstComment":"...","pillar":"...","topics_used":"..."}}]}}"""
import os, json, base64, requests, random
from flask import Blueprint, request, jsonify, Response
