...
def pick_stories(count, exclusions=None, category=None):
    ...
    if category:
        all_stories = [s for s in all_stories if s.get('category') == category]
    unused_pool = []
...
def studio():
    return Response(MJ_STUDIO_HTML.encode('utf-8'), mimetype='text/html; charset=utf-8')


@mj_bp.route('/api/generate', methods=['POST'])
def generate():
    data       = request.get_json()
    count      = min(int(data.get('count', 10)), 10)  # cap at 10 to avoid timeouts
    ...
    # Check if override corresponds to a specific story category
    PILLAR_TO_CATEGORY = {
        'philanthropy': 'philanthropy',
        'business_ventures': 'business_ventures',
        'fashion_style': 'fashion_style',
        'recording_innovation': 'recording_innovation',
        'song_histories': 'song_histories',
        'family_history': 'family_history',
        'marriages_children': 'marriages_children',
        'achievements_verified': 'achievements_verified',
        'performances_verified': 'performances_verified',
        'relationships_verified': 'relationships_verified',
    }
    # Also check the raw pillar value from request
    pillar_raw = data.get('pillar', '').strip().lower().replace(' ', '_').replace('&', 'and')
    story_category = PILLAR_TO_CATEGORY.get(pillar_raw)
    selected_stories = pick_stories(count, exclusions, category=story_category)
...
{"THEME FOCUS: " + override + ". Every post must fit within this theme, but each MUST be a DIFFERENT specific documented story angle within it  -  never repeat the same facts across posts:" if override else "Draw from these content pillars, varying across all " + str(count) + " posts:"}
{chr(10).join([f"{i+1}. Write about THIS SPECIFIC story angle: {s['angle']}" for i, s in enumerate(selected_stories)])}
...
def generate_banner():
    ...
    import random
    prop_bank = [
        'a single white sequined glove resting on dark velvet, spotlit from above',
        'a black fedora tilted on a cane, backlit with dramatic rim light',
        'a lone silhouette mid-moonwalk pose on a fog-lit stage floor',
        'a silhouette in a wide-brim hat and structured jacket, hand on hip, standing in a rain-streaked spotlight tunnel',
        'a trail of glowing white footprints leading toward a spotlight',
        'a vintage chrome microphone stand alone on an empty spotlit stage',
        'a silhouette frozen mid-spin under a single overhead spotlight, dust and light particles in the air',
        'a red zip-up jacket draped over a stage rail, lit by a single follow spot',
        'a silhouette walking away down a smoky spotlit corridor toward distant stage lights',
        'a pair of white socked ankles and black loafers caught mid-moonwalk under a spotlight, rest of figure in shadow'
    ]
    chosen_props = random.sample(prop_bank, min(count, len(prop_bank)))

    system_prompt = (
        'You are the creative director for a viral Michael Jackson tribute Facebook and Instagram page. '
        'Generate cinematic banner concepts in the style of a premium documentary or biopic poster: dramatic spotlighting, '
        'silhouettes, fog/rain atmosphere, metallic 3D typography, and symbolic stage props (never a depiction of his face or '
        'photorealistic likeness  -  silhouette and props only, so this stays image-generator safe). '
        'Banners must be moody, high-production-value, and emotionally powerful  -  think concert poster, not social media graphic. '
        'The banner text must always be a QUESTION or emotionally resonant statement that invites comments. '
    ...
    user_prompt = f"""Generate exactly {count} unique Michael Jackson cinematic banner concepts.

{"THEME FOCUS: " + override + ". Every banner must fit within this theme, but each MUST be based on a DIFFERENT specific documented story angle below  -  do not repeat the same facts across banners:" if override else "Each banner MUST be based on a DIFFERENT story angle below  -  do not combine them:"}
{chr(10).join([f"Banner {i+1}: {s['angle']}" for i, s in enumerate(stories)])}

BANNER FORMAT  -  cinematic poster style, silhouette/props only, NO face or photorealistic likeness:
- Assign each banner ONE of these prop/scene elements as the visual anchor (rotate through them, one per banner): {chr(10).join([f"  Banner {i+1}: {p}" for i, p in enumerate(chosen_props)])}
- Dark, moody backdrop  -  black or deep navy, with atmospheric fog, light rain, or floating dust/gold particles
- Dramatic spotlighting: hard directional light beams, lens flare, or a single overhead spot creating strong shadow
- Metallic, chrome, or gold 3D embossed typography with realistic light reflections  -  think movie poster title treatment, not flat social-media text
- Text stacked across 2-3 lines, large and bold, positioned to not overlap the prop/silhouette
- The text must be a QUESTION or short emotionally resonant line that invites comments
- Silhouettes must be anonymous/symbolic (no facial features, no name callouts on the figure itself)
- End description with: "Cinematic, photorealistic environment and lighting, symbolic silhouette/prop only  -  no face, no identifiable likeness. Optimized for 1080x1350 format."
...
def generate_video():
    try:
        ...
        if override:
            topic_block = "THEME FOCUS: " + override + ". Every video must fit within this theme, but each MUST use a DIFFERENT specific documented story angle below  -  do not repeat the same facts across videos:\n" + story_list
        else:
...
- Builds emotional momentum throughout — vary sentence length for rhythm
- Ends with: Follow MJ Uncovered for more stories like this
- Only documented verifiable facts — no invented details
- Write it so a narrator can read it cold with no stumbles
...
- Paragraph 1: Hook with 2-3 emojis — grabs attention
- Paragraph 2: Story development with documented facts
- Paragraph 3: Engagement CTA with 2 emojis and 5 relevant hashtags

4. FIRST COMMENT (1-2 sentences, fan-to-fan voice, documented fact + Tag a friend CTA)

5. SOURCE (name the actual documented source for this story)

JSON:
{{"videos":[{{"voiceoverScript":"...","videoScript":"...","images":[{{"num":1,"prompt":"..."}},{{"num":2,"prompt":"..."}},{{"num":3,"prompt":"..."}},{{"num":4,"prompt":"..."}},{{"num":5,"prompt":"..."}}{extra_images}],"captionAndHashtags":"...","firstComment":"...","pillar":"...","topics_used":"...","source":"..."}}]}}"""
...
def generate_carousel():
    ...
    try:
        ...
        angle = (override + "  -  specifically: " + story['angle']) if override else story['angle']
    ...
    try:
        ...
        for i, post in enumerate(posts):
            ...
            for channel_id in channel_ids:
                ...
                _time.sleep(0.7)  # ~85 requests/min max, well under 100/min limit
                # Get channel service type from channel info
                ch_info = next((c for c in channel_infos if c['id'] == channel_id), {})
                ch_service = ch_info.get('service', '').lower()

                # Instagram requires an image - queue as draft idea instead
                # so user can add image in Buffer dashboard
                if ch_service == 'instagram' and not image_url:
                    # Create as an Idea instead for Instagram (no image required)
                    idea_mutation = """
...
"""
                    ...
                    resp = requests.post(
                        ...
                        json={'query': idea_mutation, 'variables': idea_vars},
                        timeout=30
                    )
                    result = resp.json()
                    if 'errors' in result:
                        errors.append(f'Post {i+1} ig-idea: {result["errors"][0]["message"]}')
                    else:
                        idea_data = result.get('data', {}).get('createIdea', {})
                        if 'message' in idea_data:
                            errors.append(f'Post {i+1} ig-idea: {idea_data["message"]}')
                        else:
                            results.append({'postNum': i+1, 'channelId': channel_id, 'type': 'instagram_idea', 'id': idea_data.get('id')})
                    continue
...
<header>
  <div class="logo">MJ Content Studio<span>Michael Jackson Tribute Page</span><span style="font-size:9px;opacity:0.5;margin-left:8px;">v2.1</span></div>
...
<div class="layout">
  ...
  <div class="main">
    ...
    <div class="pane" id="pane-carousel">
      ...
      <button class="btn btn-dark" onclick="sendContentToBuffer('carousel')" style="font-size:12px;margin-top:8px;">Send Carousels to Buffer</button>
      </div>
    ...
    <div class="pane on" id="pane-images">
      <div class="gen-box">
        <p>Generates image posts with cinematic prompts, 3-paragraph captions, hashtags, and first comments. Pillars are randomly selected for variety. <strong style="color:var(--gold);">Tip: Generate 5-10 at a time for best results. Use "Add more" to top up.</strong></p>
        <div style="margin-bottom:10px;">
      ...
      <option value="recording_innovation">Recording Innovation</option>
        ...
        <div class="btn-row">
          ...
          <button class="btn btn-dark" onclick="downloadPromptsAsTxt()" style="font-size:12px;" title="Download all image prompts as .txt for OpenArt Bulk Create">&#8595; OpenArt Prompts</button>
          <button class="btn btn-dark" id="clear-images-btn" style="font-size:12px;">Clear</button>
        </div>
        <div id="post-selection-bar" style="display:none;margin-top:10px;padding:10px 12px;background:var(--dark2);border-radius:6px;">
          ...
          <div style="display:flex;align-items:center;gap:8px;padding-top:8px;border-top:1px solid var(--dark3);flex-wrap:wrap;">
            <span style="font-size:11px;color:var(--hint);">Add more:</span>
            <input type="number" id="add-more-count" min="1" max="19" value="5" style="width:55px;background:var(--dark1);border:1px solid var(--dark3);border-radius:4px;padding:4px 8px;font-size:12px;color:var(--text);font-family:DM Sans,sans-serif;text-align:center;">
            <span style="font-size:11px;color:var(--hint);">posts using:</span>
            <select id="add-more-override" style="background:var(--dark1);border:1px solid var(--dark3);border-radius:4px;padding:4px 8px;font-size:11px;color:var(--text);font-family:DM Sans,sans-serif;flex:1;min-width:140px;">
              <option value="">-- Random (recommended) --</option>
              <option value="Song Histories">Song Histories</option>
              <option value="Family History">Family History</option>
              <option value="Marriages &amp; Children">Marriages &amp; Children</option>
      ...
      <option value="recording_innovation">Recording Innovation</option>
        ...
        <div id="buffer-log" style="display:none;margin-top:10px;padding:10px 14px;background:var(--dark2);border:1px solid var(--dark3);border-radius:6px;font-size:11px;font-family:monospace;color:var(--muted);max-height:120px;overflow-y:auto;white-space:pre-wrap;"></div>
        <div style="display:none;">
    ...
    <div class="pane" id="pane-banners">
      ...
      <option value="recording_innovation">Recording Innovation</option>
        ...
        <div class="btn-row">
          ...
          <button class="btn btn-dark" onclick="sendContentToBuffer('banners')" style="font-size:12px;">Send to Buffer</button>
...
<div style="margin-bottom:10px;margin-top:10px;">
      ...
      <option value="recording_innovation">Recording Innovation</option>
        ...
        <div class="btn-row">
          ...
          <button class="btn btn-dark" onclick="sendContentToBuffer('short')" style="font-size:12px;">Send to Buffer</button>
          <button class="btn btn-dark" onclick="downloadVideoPromptsAsTxt('short')" style="font-size:12px;" title="Download image prompts for OpenArt">&#8595; OpenArt Prompts</button>
...
<div style="margin-bottom:10px;margin-top:10px;">
      ...
      <option value="recording_innovation">Recording Innovation</option>
        ...
        <div class="btn-row">
          <button class="btn btn-gold" id="gen-long-btn">Generate 3 Long Videos</button>
          <button class="btn btn-dark" onclick="sendContentToBuffer('long')" style="font-size:12px;">Send to Buffer</button>
          <button class="btn btn-dark" onclick="downloadVideoPromptsAsTxt('long')" style="font-size:12px;" title="Download image prompts for OpenArt">&#8595; OpenArt Prompts</button>
...
<script src="/mj/studio.js?v=3" defer></script>
</body>
</html>
ule">
      <div id="sched-container">
