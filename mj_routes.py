...
@mj_bp.route('/api/story-export', methods=['GET'])
def story_export():
    """Export full story list with used/unused status as CSV"""
    try:
        all_stories = load_stories()
        used_ids = load_used_ids()
        rows = ['ID,Category,Status,Angle']
        for s in sorted(all_stories, key=lambda x: (x['category'], x['id'])):
            status = 'Used' if s['id'] in used_ids else 'Unused'
            angle = s['angle'].replace('"', '""')
            row = '"' + s['id'] + '","' + s['category'] + '","' + status + '","' + angle + '"'
            rows.append(row)
        csv_out = chr(10).join(rows)
        from flask import Response as _Resp
        return _Resp(csv_out, mimetype='text/csv',
                     headers={'Content-Disposition': 'attachment; filename=mj_story_database.csv'})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
<div id="settings-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:500;align-items:center;justify-content:center;">
  <div style="background:var(--dark2);border:1px solid var(--border);border-radius:12px;padding:32px;width:460px;max-width:90vw;">
    ...
    <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--dark3);">
      ...
      <button id="export-stories-btn" onclick="exportStoryDatabase()" class="btn btn-dark" style="width:100%;margin-top:8px;font-size:12px;">&#8595; Export Story Stats (CSV)</button>
    </div>
...
<script src="/mj/studio.js?v=4" defer></script>
</body>
</html>
'''
