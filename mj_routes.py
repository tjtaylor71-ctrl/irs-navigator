...
function renderVideos(type) {
  ...
  arr.forEach((v, i) => {
    ...
    if (imgs.length) {
      ...
      let imgHTML = '<div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:10px;">' + imgs.length + ' IMAGE PROMPTS - 9:16 for Seedance/Kling</div>';
      imgs.forEach(function(img) {
        ...
        imgHTML += '<div style="font-size:12px;color:var(--muted);line-height:1.6;font-style:italic;margin-bottom:4px;">' + esc(img.prompt) + '</div>';
        imgHTML += '<button class="vid-copy-prompt mbtn" data-prompt="' + encodeURIComponent(img.prompt || '') + '" style="font-size:10px;">Copy</button>';
        imgHTML += '</div>';
      });
      const allText = imgs.map(function(img){ return 'Image ' + img.num + ':\n' + (img.prompt||''); }).join('\n\n');
      imgHTML += '<button class="vid-copy-all mbtn" data-prompts="' + encodeURIComponent(allText) + '" style="border:1px solid var(--gold-dim);color:var(--gold);font-size:10px;">Copy All ' + imgs.length + ' Prompts</button>';
