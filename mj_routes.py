...
def generate():
    ...
    system_prompt = (
        ...
        '1. NEVER invent quotes. Only use quotes documented in named interviews, books, or recordings. '
        '2. NEVER describe private on-set moments or personal encounters unless documented in a named publication or documentary. '
        '3. NEVER use phrases like "almost lost to history," "cameras were not rolling," "rare moment no one saw," '
        'or any language implying secret or unverified footage. '
        '4. NEVER invent specific details like ages, durations, or dialogue to make a story feel authentic. '
        '5. Every claim must be verifiable — if a fan Googled it, they should find it confirmed. '
        'WHAT IS ALLOWED: Documented performances (name the show, year, venue). Published interviews with named outlets. '
        'Confirmed facts from named biographies and documentaries. Verified record achievements. '
        'Documented humanitarian work. Real production stories from named collaborators. '
        'Rare and compelling content works because the REAL story is extraordinary enough — never fabricate. '
        'REAL-PERSON LIKENESS RULE: All image prompts must anchor Michael Jackson authentic likeness to the '
        'correct era with specific physical descriptors. Always state it is the real Michael Jackson. '
...
{"Theme override: " + override + ". " if override else ""}Draw from these content pillars, varying across all {count} posts:
{chr(10).join([f"{i+1}. {p}" for i, p in enumerate(selected_pillars[:count])])}

IMAGE PROMPT FORMAT — STRICT VISUAL VARIETY REQUIRED:
Every image must depict a COMPLETELY DIFFERENT scene type. Rotate through these scene categories so no two posts in a batch share the same type:
- Studio/recording session (at the mixing board, in the booth, headphones on, working with producers)
- Portrait/fashion (close-up, editorial style, iconic outfit detail, the glove, the fedora, the jacket)
- Backstage/preparation (getting ready, costume, mirror reflection, pre-show quiet moment)
- Humanitarian/public appearance (charity event, meeting fans, public speech, award ceremony)
- Rehearsal (dance studio, learning choreography, working with dancers)
- Candid/lifestyle (reading, relaxing, with family, natural moment documented in named publications)
- Album/era artistic (concept art style matching a specific album era, not a concert)
- Interview/press (documented press appearance, named publication photoshoot era)
- Concert ONLY if the pillar specifically calls for it — do NOT default to concert for every post

Each image prompt must:
- Anchor Michael Jackson authentic likeness to the correct era with specific physical descriptors (age, hairstyle, specific outfit)
- State it is the real Michael Jackson (not an actor or tribute artist)
- Use the scene category that best fits the assigned pillar
...
For each post also include: "pillar" (1-word summary of which pillar was used)
...
def generate_video():
    ...
    system_prompt = (
        ...
        'Generate cinematic, emotionally powerful spoken narration scripts. '
        'ABSOLUTE RULES — never violate these: '
        '1. NEVER invent quotes. Only use documented quotes with a named source. '
        '2. NEVER describe private moments, secret interactions, or unverified on-set stories. '
        '3. NEVER use language like "almost lost to history," "cameras were not rolling," '
        '"a moment no one was supposed to see," or similar fabrication signals. '
        '4. NEVER invent specific details — ages, durations, exact words — to make a story feel real. '
        '5. Every claim must be verifiable. If a fan Googled it, they should find it confirmed. '
        'WHAT IS ALLOWED: Documented performances with named venues and dates. Published interviews '
        'with named outlets (Rolling Stone, Ebony, Oprah, Bashir documentary). Confirmed facts from '
        'named biographies. Verified record achievements with sources. Documented humanitarian work. '
        'Real production stories from named collaborators. '
        'The real MJ story is extraordinary enough — it never needs fabrication. '
...
async function generateImages() {
  ...
  try {
    const res = await fetch('/mj/api/generate', {
      ...
      body: JSON.stringify({count, themeOverride: override, tone: S.tone, cta: S.cta})
    });
...
function buildCard(type, idx, item, time, typeLabel, typeCls, s1text, s2text, s3text, tab1label, tab2label) {
  ...
  return '<div class="card-head">'
    ...
    +'<button class="mbtn" data-action="copy" data-type="'+type+'" data-idx="'+idx+'" data-field="'+(type==='img'?'caption':'captionAndHashtags')+'">Caption</button>'
