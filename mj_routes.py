  ...
  <div class="main">
    ...
    <div class="pane on" id="pane-images">
      <div class="theme-box">
        ...
        <div class="btn-row">
          <button class="btn btn-gold" id="gen-img-btn">Generate 19 Image Posts</button>
...
function buildCardHTML(type, idx, item, time, typeLabel, typeCls, s1text, s2text, s3text, tabLabels, tabFields) {
  ...
  const tabsHTML = tabLabels.map((label,ti) => '<div class="sec-tab '+(ti===0?'on':'')+'" data-action="sectab" data-panes="'+allPanes+'" data-show="'+type+'-s'+(ti+1)+'-'+idx+'">'+label+'</div>').join('');
  const texts = [s1text, s2text, s3text];
  const panesHTML = tabLabels.map((_,ti) => '<div class="sec-pane '+(ti===0?'on':'')+'" id="'+type+'-s'+(ti+1)+'-'+idx+'"><div class="section-text'+(ti===0&&type==='img'?' prompt':'')+'">'+(ti===0?esc(s1text||''):ti===1?esc(s2text||''):esc(s3text||''))+'</div></div>').join('');
