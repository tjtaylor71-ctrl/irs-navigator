...
- If the story is about a SONG, depict a recording studio session, a relevant era performance moment, or album artwork era - matching that song's time period.
- If the story is about FAMILY/CHILDREN, the image MUST include the relevant family member(s) described (e.g. if the story is about Paris's memories of her father, show Michael Jackson WITH his daughter Paris, or a father-daughter scene appropriate to the era).
- If the story is about a MARRIAGE, the image should reflect that relationship/era appropriately (e.g. MJ and Lisa Marie Presley together if the story is about their marriage).
- If the story is about a COLLABORATION, depict MJ with or alongside the named collaborator, or a studio/creative scene reflecting that collaboration.
- If the story is about an ACHIEVEMENT or PERFORMANCE, depict that specific performance/moment's era and setting.
- If the story is about FAMILY HISTORY (Jackson 5, Gary Indiana, Motown), depict the appropriate young MJ era with siblings/family as relevant.
Vary scene TYPE (portrait, candid, studio, performance, family moment, etc.) based on what fits the story - but the scene must always directly illustrate the story's actual subject matter, not be generic.
Each image must anchor Michael Jackson authentic likeness to the correct era with specific physical descriptors.
State it is the real Michael Jackson (not an actor or tribute artist). If other real people appear (family members, collaborators), name them and describe them accurately for their era too.
ONLY use concert/stage scenes if the story angle is specifically about a performance.
...
JSON: {{"posts":[{{"imagePrompt":"...","caption":"...","firstComment":"...","pillar":"...","topics_used":"...","source":"[name the actual source - interview/publication/documentary/year this story comes from]"}}]}}"""
    ...
    try:
        resp = requests.post(
            ...
            json={'model':'claude-sonnet-4-6','max_tokens':8000,'system':system_prompt,
...
- "imagePrompt": A cinematic still image prompt depicting THE ACTUAL SUBJECT of this script's story angle (not a generic MJ shot). If the story involves family members, collaborators, or other named people, include them in the scene accurately for their era. The scene must directly illustrate what the script is about. Include Michael Jackson's authentic likeness anchored to the correct era with specific physical descriptors. State it is the real Michael Jackson. End with "Optimized for 1080x1350 format."
...
def refresh_stories():
    ...
    try:
        ...
        categories = {
            "song_histories": 100,
            "family_history": 60,
            "marriages_children": 40,
            "achievements_verified": 40,
            "performances_verified": 40,
            "relationships_verified": 40,
        }
