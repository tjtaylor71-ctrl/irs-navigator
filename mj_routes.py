...
# Day-of-week themes
DAY_THEMES = {
    0: {'name': 'Moonwalk Monday',      'focus': 'documented dance innovations, the moonwalk debut on Motown 25 (May 16 1983), documented choreography evolution, publicly recorded rehearsal footage'},
    1: {'name': 'Thriller Tuesday',     'focus': 'Thriller era documented facts — album release, chart records, Grammy wins, John Landis music video, Vincent Price voiceover, documented cultural impact'},
    2: {'name': 'Deep Cut Wednesday',   'focus': 'lesser-known but fully documented songs, B-sides, documented album tracks, confirmed live performances of deep cuts, documented collaborations'},
    3: {'name': 'Thriller to HIStory Thursday', 'focus': 'documented evolution across Bad, Dangerous and HIStory eras — confirmed tours, documented album milestones, publicly recorded transformation'},
    4: {'name': 'Live Performance Friday', 'focus': 'documented stadium concerts, confirmed TV performances, recorded live moments — Grammy Awards, Super Bowl XXVII, Motown 25, Victory Tour, Bad Tour, Dangerous Tour, HIStory Tour'},
    5: {'name': 'Style and Aura Saturday', 'focus': 'documented fashion choices, publicly photographed iconic outfits, confirmed collaborations with designers, documented style evolution across eras'},
    6: {'name': 'Emotional Legacy Sunday', 'focus': 'documented humanitarian work — Heal the World Foundation, We Are the World, confirmed charity work, publicly documented legacy, This Is It concert series'},
}
...
def generate():
    ...
    system_prompt = (
        ...
        'Apply strict anti-repetition logic — vary eras, moods, and storytelling angles across every post. '
...
- Optimized for Hypernatural"""
    ...
    system_prompt = (
        ...
        'CRITICAL: Only reference events, performances, and facts that are widely documented and publicly verifiable. '
        'Never speculate about private moments or invent unverified details. '
        'Base all content on confirmed historical record — documented tours, recorded performances, '
        'verified chart achievements, confirmed collaborations, and publicly known facts about his life and career. '
        'Apply strict anti-repetition logic across all scripts. '
        'Respond ONLY with valid JSON. No markdown. No code fences. No preamble.'
    )
...
const DAY_THEMES = {
  ...
  2:{name:'Deep Cut Wednesday',focus:'lesser-known but fully documented songs, B-sides, confirmed live performances, documented collaborations'},
...
function clearSavedPosts() {
  localStorage.removeItem('mj_imagePosts');
  localStorage.removeItem('mj_shortVideos');
  localStorage.removeItem('mj_longVideos');
...
function applyImage(type,idx,dataUrl) {
  ...
  updateStats(); savePosts(); renderSchedule(); toast('Image set for #'+(idx+1));
}
...
async function scheduleItem(type,idx) {
  ...
  try {
    ...
    updateStats(); savePosts(); renderSchedule(); toast((type==='img'?'Post':'Video')+' #'+(idx+1)+' scheduled!');
