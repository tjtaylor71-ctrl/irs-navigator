...
function buildCardHTML(type, idx, item, time, typeLabel, typeCls) {
  ...
  if (type === 'img') {
    ...
    let out = '<div style="margin:0 0 12px;padding:12px 16px;background:var(--dark2);border-radius:8px;">';
    ...
    imgs.forEach(function(img) {
      ...
      out += '<div class="vid-img-prompt" style="font-size:12px;color:var(--muted);line-height:1.6;font-style:italic;">' + esc(img.prompt) + '</div>';
      out += '<button class="vid-copy-prompt mbtn" data-prompt="' + encodeURIComponent(img.prompt) + '" style="margin-top:4px;font-size:10px;">Copy Prompt</button>';
      out += '</div>';
    });
    const allPrompts = imgs.map(function(img){ return 'Image ' + img.num + ':\n' + img.prompt; }).join('\n\n');
    out += '<button class="vid-copy-all mbtn" data-prompts="' + encodeURIComponent(allPrompts) + '" style="margin-top:4px;border:1px solid var(--gold-dim);color:var(--gold);font-size:10px;">Copy All ' + imgs.length + ' Prompts</button>';
    out += '</div>';
    return out;
  })() : '';
